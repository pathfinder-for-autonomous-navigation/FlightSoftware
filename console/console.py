...
if __name__ == '__main__':
    ...
    with open("pan_logo.txt", "r") as pan_logo:
        if args.port_fc:
            fc_cmd = StateSession(args.port_fc, 1152000, args.data_dir, "FlightComputer")
            if not fc_cmd.connect():
                fc_cmd = None
        if args.port_adcs:
            adcs_cmd = StateSession(args.port_adcs, 1152000, args.data_dir, "ADCSComputer")
            if not adcs_cmd.connect():
                adcs_cmd = None
