...
platformio test -e fsw_downlink_parser_ci -v

# Check for memory mismanagement
valgrind --leak-check=full --show-leak-kinds=all --track-origins=yes --verbose .pio/build/fsw_downlink_parser_ci/program

# Compile and run functional test for environments
platformio run -e fsw_downlink_parser
