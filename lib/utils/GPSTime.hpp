...
static constexpr unsigned long NANOSECONDS_IN_WEEK =
    MILLISECONDS_IN_WEEK * 1000000;
...
struct gps_time_t {
  ...
  bool operator!=(const gps_time_t &t1) {
    return t1.gpstime.wn != gpstime.wn || t1.gpstime.tow != gpstime.tow ||
           t1.gpstime.ns != gpstime.ns;
  }

  /** Inequality operator **/
  bool operator!=(const unsigned long &t1) {
    return (gpstime.wn * NANOSECONDS_IN_WEEK) + (gpstime.tow * 1000 * 1000) !=
           t1;
  }

  /** Equality operator **/
  bool operator==(const gps_time_t &t1) { return !(this->operator!=(t1)); }

  /** Equality operator **/
  bool operator==(const unsigned int &t1) { return !(this->operator!=(t1)); }
  ...
  bool operator<(const gps_time_t &t1) {
    return gpstime.wn < t1.gpstime.wn ||
           ...
           (gpstime.wn == t1.gpstime.wn && gpstime.tow == t1.gpstime.tow &&
            gpstime.ns == t1.gpstime.ns);
  }

  /** Comparison operator **/
  bool operator<(const unsigned long &t1) {
    return gpstime.wn * NANOSECONDS_IN_WEEK + gpstime.tow * 1000000 +
               gpstime.ns <
           t1;
  }
