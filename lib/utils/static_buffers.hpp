...
**/
template <typename T, size_t S> class static_buffer {
protected:
  T buf_[S]; // buffer
  T *head_ = buf_;
  T *tail_ = buf_;
...
template <typename T, size_t S> static_buffer<T, S>::static_buffer() {}

template <typename T, size_t S> void static_buffer<T, S>::put(T item) {
  *head_ = item;
  if (full_)
    advance_ptr_(tail_);
  advance_ptr_(head_);
...
template <typename T, size_t S> void static_buffer<T, S>::reset() {
  head_ = tail_;
  full_ = false;
}

template <typename T, size_t S> bool static_buffer<T, S>::empty() const {
  // if head and tail are equal, we are empty
  return (!full_ && (head_ == tail_));
}

template <typename T, size_t S> bool static_buffer<T, S>::full() const {
  // If tail is ahead the head by 1, we are full
  return full_;
}

template <typename T, size_t S> size_t static_buffer<T, S>::size() const {
  return S;
}

template <typename T, size_t S> T &circular_buffer<T, S>::get() {
  if (static_buffer<T, S>::empty())
    return *(new T);
...
template <typename T, size_t S> T &circular_stack<T, S>::get() {
  if (static_buffer<T, S>::empty())
    return *(new T);
