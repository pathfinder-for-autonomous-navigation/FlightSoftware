...
/**
 * @brief A control ControlTaskBase is a wrapper around any high-level
 * ControlTaskBase that interacts with satellite data.
 *
 ...
 */
template <typename T> class ControlTask : protected debug_console {
public:
  /**
...
protected:
  ...
  template <typename U> bool add_internal_field(InternalStateField<U> &field) {
    return _registry.add_internal_field(&field);
  }

  template <typename U> bool add_readable_field(ReadableStateField<U> &field) {
    return _registry.add_readable_field(&field);
  }

  template <typename U> bool add_writable_field(WritableStateField<U> &field) {
    return _registry.add_writable_field(&field);
  }
  ...
  InternalStateField<U> *find_internal_field(const char *field,
    ...
    if (!field_ptr) {
      printf(debug_severity::error,
             "%s:%d: Internal field required is not present in state registry: "
             "%s\n",
             file, line, field);
    }
  ...
  ReadableStateField<U> *find_readable_field(const char *field,
    ...
    if (!field_ptr) {
      printf(debug_severity::error,
             "%s:%d: Readable field required is not present in state registry: "
             "%s\n",
             file, line, field);
    }
  ...
  WritableStateField<U> *find_writable_field(const char *field,
    ...
    if (!field_ptr) {
      printf(debug_severity::error,
             "%s:%d: Writable field required is not present in state registry: "
             "%s\n",
             file, line, field);
    }
...
template <typename T> ControlTask<T>::~ControlTask() {}
