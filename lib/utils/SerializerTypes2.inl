...
class Serializer<unsigned int> : public SerializerBase<unsigned int> {
   public:
    using SerializerBase<unsigned int>::SerializerBase;
...
class Serializer<signed int> : public SerializerBase<signed int> {
   public:
    using SerializerBase<signed int>::SerializerBase;
 ...
 */
template <>
class Serializer<temperature_t> : public Serializer<signed int> {
   public:
    ...
    Serializer()
        : Serializer<signed int>(TEMPERATURE_MIN, TEMPERATURE_MAX, SerializerConstants::temp_sz) {}

    void serialize(const temperature_t &src) {
        Serializer<signed int>::serialize(static_cast<signed int>(src));
    }

    void deserialize(std::shared_ptr<temperature_t> &dest) {
        Serializer<signed int>::deserialize(dest);
    }
};
...
class Serializer<float> : public SerializerBase<float> {
   public:
    using SerializerBase<float>::SerializerBase;
...
class Serializer<double> : public SerializerBase<double> {
   public:
    using SerializerBase<double>::SerializerBase;
...
class Serializer<std::array<float, N>> : public SerializerBase<std::array<float, N>> {
   protected:
     ...
     */
    std::array<std::unique_ptr<Serializer<float>>, N - 1> vector_element_serializer;
   ...
   public:
     ...
     */
    Serializer(float min = 0.0, float max = 1.0, size_t size = f_quat_sz)
        : SerializerBase<f_vector_t>(dummy_vector, dummy_vector, size),
          magnitude_min(min),
          magnitude_max(max),
          magnitude_serializer(
              std::make_unique<Serializer<float>>(min, max, size - f_vec_min_size)) {
        max_component.resize(2);
        for (size_t i = 0; i < N - 1; i++) {
            component_scaled_values[i].resize(f_vec_quat_component_sz);
            vector_element_serializer[i] =
                std::make_unique<Serializer<float>>(0.0f, sqrtf(2.0f), f_vec_quat_component_sz);
        }
    }

    void serialize(const std::array<float, N> &src) {
        auto serialized_position = serialized_val.begin();
        ...
        for (int i = 0; i < N; i++) {
            if (v_mags[i] == max_element_value) {
                ...
                element_serializers[component_number]->serialize(element_scaled);
                std::copy(element_serializers[component_number]->get_bit_array().begin(),
                          element_serializers[component_number]->get_bit_array().end(),
                          serialized_position);
                component_number++;
            }
        }
    ...
    void deserialize(std::shared_ptr<std::array<float, N>> &dest) {
        ...
        unsigned int missing_component = (serialized_val[magnitude_serializer->bitsize()] << 1) +
                                         serialized_val[magnitude_serializer->bitsize() + 1];
        ...
        for (int i = 0; i < N; i++) {
            if (i != missing_component) {
                ...
                vector_element_serializer[i]->deserialize(component_ptr);
                j++;
            }
        }
...
class Serializer<gps_time_t> : public SerializerBase<gps_time_t> {
   protected:
    static gps_time_t dummy_gpstime;
