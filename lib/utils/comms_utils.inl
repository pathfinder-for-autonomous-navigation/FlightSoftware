...
typename std::enable_if<(max_vec_size > Comms::MAX_NORMALIZED_FLOAT_VECTOR_SIZE),
                        void>::type inline Comms::trim_vector(const std::array<float, 3>& v,
                                                              float min_magnitude,
                                                              float max_magnitude,
    ...
    for (int i = 0; i < 3; i++) {
        if (i == max_element_idx) {
            ...
            trim_float(v_element_scaled, -sqrt(2), sqrt(2),
...
template <size_t max_vec_size>
typename std::enable_if<(max_vec_size > Comms::MAX_NORMALIZED_DOUBLE_VECTOR_SIZE),
                        void>::type inline Comms::trim_vector(const std::array<double, 3>& v,
    ...
    for (int i = 0; i < 3; i++) {
        if (i == max_element_idx) {
            ...
            trim_double(v_element_scaled, -sqrt(2), sqrt(2),
                        &vec_element_representations[vec_number]);
            vec_number++;
        }
    }
...
template <size_t max_vec_size>
typename std::enable_if<(max_vec_size > Comms::MAX_NORMALIZED_FLOAT_VECTOR_SIZE),
                        void>::type inline Comms::expand_vector(const std::bitset<max_vec_size>& v,
                                                                float min_magnitude,
                                                                float max_magnitude,
    ...
    for (int i = 0; i < 3; i++) {
        if (i != missing_component) {
            ...
            for (int k = 0; k < MAX_FLOAT_VECTOR_COMPONENT_SIZE; k++)
                vec_component_packed.set(k, v[max_vec_size - MAX_NORMALIZED_FLOAT_VECTOR_SIZE + 2 +
                                              j * MAX_FLOAT_VECTOR_COMPONENT_SIZE + k]);
    ...
    for (int i = 0; i < 3; i++) (*result)[i] *= magnitude;
}
...
typename std::enable_if<(max_vec_size > Comms::MAX_NORMALIZED_DOUBLE_VECTOR_SIZE),
    ...
    for (int i = 0; i < 3; i++) {
        if (i != missing_component) {
            ...
            for (int k = 0; k < MAX_DOUBLE_VECTOR_COMPONENT_SIZE; k++)
                vec_component_packed.set(k, v[max_vec_size - MAX_NORMALIZED_DOUBLE_VECTOR_SIZE + 2 +
                                              j * MAX_FLOAT_VECTOR_COMPONENT_SIZE + k]);
