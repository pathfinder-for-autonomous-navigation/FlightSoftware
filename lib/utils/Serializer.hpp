...
template <typename U>
class SerializerBase : public InitializationRequired,
                       public SerializerConstants {
protected:
  U _min;
  U _max;
 ...
 */
template <>
class Serializer<bool, bool, SerializerConstants::bool_sz>
    : public SerializerBase<bool> {
public:
...
class Serializer<unsigned int, unsigned int, compressed_sz>
    : public SerializerBase<unsigned int> {
public:
...
class Serializer<signed int, signed int, compressed_sz>
    : public SerializerBase<signed int> {
public:
...
public:
  ...
  bool serialize(const f_vector_t &src, std::bitset<compressed_sz> *dest) {
    ...
    float mag =
        sqrtf(powf(src[0], 2.0f) + pow(src[1], 2.0f) + pow(src[2], 2.0f));
    ...
    float max_element_value =
        std::max(std::max(v_mags[0], v_mags[1]), v_mags[2]);
    ...
    for (int i = 0; i < 3; i++) {
      if (i == max_element_idx) {
        ...
        vector_element_serializer.serialize(
            v_element_scaled, &vec_element_representations[vec_number]);
        vec_number++;
      }
    }
  ...
  bool deserialize(const std::bitset<compressed_sz> &src, f_vector_t *dest) {
    ...
    unsigned int missing_component =
        (src[magnitude_bitsize] << 1) + src[magnitude_bitsize + 1];
    ...
    for (int i = 0; i < 3; i++) {
      if (i != missing_component) {
        ...
        for (int k = 0; k < f_vec_component_sz; k++)
          vec_component_packed.set(k, src[compressed_sz - f_vec_min_sz + 2 +
                                          j * f_vec_component_sz + k]);

        Serializer<float, float, f_vec_component_sz> vec_component_serializer;
        vec_component_serializer.init(0.0f, sqrtf(2.0f));
        vec_component_serializer.deserialize(vec_component_packed,
                                             &((*dest)[i]));
        j++;
      }
    }
  ...
  bool serialize(const d_vector_t &src, std::bitset<compressed_sz> *dest) {
    ...
    for (int i = 0; i < 3; i++) {
      if (i == max_element_idx) {
        ...
        Serializer<double, double, d_vec_component_sz>
            vector_element_serializer;
    ...
    for (int i = 0; i < 3; i++) {
      if (i != missing_component) {
        ...
        for (int k = 0; k < d_vec_component_sz; k++)
          vec_component_packed.set(k, src[compressed_sz - d_vec_component_sz +
                                          2 + j * d_vec_component_sz + k]);
 ...
 */
template <>
class Serializer<f_quat_t, float, SerializerConstants::f_quat_sz>
    : public SerializerBase<float> {
public:
  Serializer() : SerializerBase<float>(0.0f, 0.0f) {}
  ...
  bool serialize(const f_quat_t &src, std::bitset<f_quat_sz> *dest) {
    ...
    int max_element_idx = 0;
    float max_element_value = std::max(std::max(q_mags[0], q_mags[1]),
                                       std::max(q_mags[2], q_mags[3]));
    ...
    for (int i = 0; i < 4; i++) {
      if (i == max_element_idx) {
        ...
        quat_element_serializer.serialize(
            src[quat_number], &quat_element_representations[quat_number]);
        quat_number++;
      }
    }
    for (int i = 0; i < 9; i++) {
 ...
 */
template <>
class Serializer<d_quat_t, double, SerializerConstants::d_quat_sz>
    : public SerializerBase<double> {
public:
  Serializer() : SerializerBase<double>(0.0, 0.0) {}
 ...
 */
template <>
class Serializer<gps_time_t, bool, SerializerConstants::gps_time_sz>
    : public SerializerBase<bool> {
public:
...
public:
  static constexpr temperature_t TEMPERATURE_MIN =
      static_cast<temperature_t>(-40);
  static constexpr temperature_t TEMPERATURE_MAX =
      static_cast<temperature_t>(125);

  Serializer()
      : SerializerBase<temperature_t>(TEMPERATURE_MIN, TEMPERATURE_MAX) {}
