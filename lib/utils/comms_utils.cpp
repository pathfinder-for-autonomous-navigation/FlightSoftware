...
void Comms::trim_gps_time(const gps_time_t &gpstime, std::bitset<Comms::GPSTIME_SIZE> *result) {
    ...
    for (int i = 0; i < 16; i++) result->set(i + 1, wn[i]);
    for (int i = 0; i < 32; i++) result->set(i + 17, tow[i]);
}

void Comms::expand_gps_time(const std::bitset<Comms::GPSTIME_SIZE> &gpstime, gps_time_t *result) {
    std::bitset<16> wn;
    std::bitset<32> tow;
    for (int i = 0; i < 16; i++) wn.set(i + 1, gpstime[i]);
    for (int i = 0; i < 32; i++) tow.set(i + 1, gpstime[16 + i]);
