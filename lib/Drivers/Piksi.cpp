...
bool Piksi::setup() {
  ...
  registration_successful |=
      sbp_register_callback(&_sbp_state, SBP_MSG_LOG, &Piksi::_log_callback,
                            this, &Piksi::_log_callback_node);
  registration_successful |= sbp_register_callback(
      &_sbp_state, SBP_MSG_GPS_TIME, &Piksi::_gps_time_callback, this,
      &Piksi::_gps_time_callback_node);
  registration_successful |=
      sbp_register_callback(&_sbp_state, SBP_MSG_DOPS, &Piksi::_dops_callback,
                            this, &Piksi::_dops_callback_node);
  registration_successful |= sbp_register_callback(
      &_sbp_state, SBP_MSG_POS_ECEF, &Piksi::_pos_ecef_callback, this,
      &Piksi::_pos_ecef_callback_node);
  registration_successful |= sbp_register_callback(
      &_sbp_state, SBP_MSG_BASELINE_ECEF, &Piksi::_baseline_ecef_callback, this,
      &Piksi::_baseline_ecef_callback_node);
  registration_successful |= sbp_register_callback(
      &_sbp_state, SBP_MSG_VEL_ECEF, &Piksi::_vel_ecef_callback, this,
      &Piksi::_vel_ecef_callback_node);
  registration_successful |= sbp_register_callback(
      &_sbp_state, SBP_MSG_BASE_POS_ECEF, &Piksi::_base_pos_ecef_callback, this,
      &Piksi::_base_pos_ecef_callback_node);
  registration_successful |= sbp_register_callback(
      &_sbp_state, SBP_MSG_IAR_STATE, &Piksi::_iar_callback, this,
      &Piksi::_iar_callback_node);
  registration_successful |=
  ...
  registration_successful |= sbp_register_callback(
      &_sbp_state, SBP_MSG_HEARTBEAT, &Piksi::_heartbeat_callback, this,
      &Piksi::_heartbeat_callback_node);
  registration_successful |= sbp_register_callback(
      &_sbp_state, SBP_MSG_STARTUP, &Piksi::_startup_callback, this,
      &Piksi::_startup_callback_node);
  registration_successful |= sbp_register_callback(
      &_sbp_state, SBP_MSG_UART_STATE, &Piksi::_uart_state_callback, this,
      &Piksi::_uart_state_callback_node);
  registration_successful |= sbp_register_callback(
      &_sbp_state, SBP_MSG_USER_DATA, &Piksi::_user_data_callback, this,
...
void Piksi::get_baseline_ecef(unsigned int *tow,
...
#ifdef DESKTOP
void Piksi::set_gps_time(const unsigned int tow) { _gps_time.tow = tow; }
void Piksi::set_pos_ecef(const unsigned int tow,
                         const std::array<double, 3> &position,
                         const unsigned char nsats) {
  _pos_ecef.tow = tow;
...
void Piksi::set_vel_ecef(const unsigned int tow,
...
void Piksi::set_baseline_ecef(const unsigned int tow,
                              const std::array<double, 3> &position) {
...
void Piksi::set_read_return(const unsigned int out) { _read_return = out; }
#endif
...
float Piksi::get_uart_a_tx_throughput() {
  return _uart_state.uart_a.tx_throughput;
}
float Piksi::get_uart_a_rx_throughput() {
  return _uart_state.uart_a.rx_throughput;
}
unsigned short int Piksi::get_uart_a_crc_error_count() {
  return _uart_state.uart_a.crc_error_count;
}
unsigned short int Piksi::get_uart_a_io_error_count() {
  return _uart_state.uart_a.io_error_count;
}
...
float Piksi::get_uart_b_tx_throughput() {
  return _uart_state.uart_b.tx_throughput;
}
float Piksi::get_uart_b_rx_throughput() {
  return _uart_state.uart_b.rx_throughput;
}
unsigned short int Piksi::get_uart_b_crc_error_count() {
  return _uart_state.uart_b.crc_error_count;
}
unsigned short int Piksi::get_uart_b_io_error_count() {
  return _uart_state.uart_b.io_error_count;
}
...
void Piksi::settings_save() {
  sbp_send_message(&_sbp_state, SBP_MSG_SETTINGS_SAVE, SBP_SENDER_ID, 0,
                   nullptr, &Piksi::_uart_write);
}
void Piksi::settings_write(const msg_settings_write_t &settings) {
  sbp_send_message(&_sbp_state, SBP_MSG_SETTINGS_WRITE, SBP_SENDER_ID,
                   sizeof(msg_settings_write_t), (unsigned char *)&settings,
                   &Piksi::_uart_write);
}
void Piksi::piksi_reset() {
  sbp_send_message(&_sbp_state, SBP_MSG_RESET, SBP_SENDER_ID, 0, nullptr,
                   &Piksi::_uart_write);
}
void Piksi::send_user_data(const msg_user_data_t &data) {
  sbp_send_message(&_sbp_state, SBP_MSG_USER_DATA, SBP_SENDER_ID,
                   sizeof(msg_user_data_t), (unsigned char *)&data,
                   &Piksi::_uart_write);
}

signed char Piksi::process_buffer() {
  signed char status =
      ((signed char)sbp_process(&_sbp_state, Piksi::_uart_read));
  return status;
}
...
unsigned char Piksi::read_all() {
  ...
  if (bytes_available()) {
    ...
    // ensure that if the while loop terminated because of exceeding the
    // READ_ALL_LIMIT it will enter the clear bytes condition below
    delayMicroseconds(5);
    ...
    // by this point in the code, it is guarenteed that there are no more bytes
    // in buffer

    if (crc_error)
      return 3;
    else if (_gps_time_update && _pos_ecef_update && _vel_ecef_update &&
             !_baseline_ecef_update)
      // SPP
      return 0;
    else if (_gps_time_update && _pos_ecef_update && _vel_ecef_update &&
             _baseline_ecef_update)
...
void Piksi::_insert_log_msg(u8 msg[]) {
  ...
  if (_logbook_size < _logbook_max_size)
    _logbook_size++;
}
...
void Piksi::_baseline_ecef_callback(u16 sender_id, u8 len, u8 msg[],
                                    void *context) {
  Piksi *piksi = (Piksi *)context;
...
void Piksi::_base_pos_ecef_callback(u16 sender_id, u8 len, u8 msg[],
                                    void *context) {
  Piksi *piksi = (Piksi *)context;
...
void Piksi::_settings_read_resp_callback(u16 sender_id, u8 len, u8 msg[],
                                         void *context) {
  Piksi *piksi = (Piksi *)context;
  memcpy((u8 *)(&(piksi->_settings_read_resp)), msg,
         sizeof(msg_settings_read_resp_t));
}
void Piksi::_heartbeat_callback(u16 sender_id, u8 len, u8 msg[],
                                void *context) {
  Piksi *piksi = (Piksi *)context;
...
void Piksi::_uart_state_callback(u16 sender_id, u8 len, u8 msg[],
                                 void *context) {
  Piksi *piksi = (Piksi *)context;
  memcpy((u8 *)(&(piksi->_uart_state)), msg, sizeof(msg_uart_state_t));
}
void Piksi::_user_data_callback(u16 sender_id, u8 len, u8 msg[],
                                void *context) {
  Piksi *piksi = (Piksi *)context;
