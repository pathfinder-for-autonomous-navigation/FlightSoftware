...
void Piksi::write_default_settings() {
  ...
  const char *broadcast_surveyed_position =
      "surveyed position\000broadcast\000true";
  // RTK solution settings.
  const char *dgnss_solution_mode =
      "solution\000dgnss solution mode\000TimeMatched";
  // Hardware watchdog.
  const char *enable_hardware_watchdog = "system monitor\000watchdog\000true";
  // Heartbeat output rate.
  const char *heartbeat_period =
      "system monitor\000heartbeat period milliseconds\00010000";
  // Telemetry radio settings
  const char *telemetry_settings = "telemetry radio\000configuration "
...
void Piksi::_settings_read_resp_callback(u16 sender_id, u8 len, u8 msg[],
                                         void *context) {
  Piksi *piksi = (Piksi *)context;
  memcpy((u8 *)(&(piksi->_base_pos_ecef)), msg,
