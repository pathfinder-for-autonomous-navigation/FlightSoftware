...
const std::array<unsigned char, 6> PropulsionSystem::valve_pins = {27, 28, 3,
                                                                   4,  5,  6};

volatile bool PropulsionSystem::is_valve_opened[4] = {false, false, false,
                                                      false};

volatile unsigned int PropulsionSystem::thrust_valve_schedule[4] = {0, 0, 0, 0};
...
// TODO
  ...
  thrust_valve_loop_timer.begin(thrust_valve_loop,
                                thrust_valve_loop_interval_us);
#endif
  return true;
}
...
void PropulsionSystem::set_thrust_valve_schedule(
    const std::array<unsigned int, 4> &setting) {
  disable();
#ifndef DESKTOP
  delayMicroseconds(
      10); // Wait for current cycle of the thrust valve loop to end
#endif
  for (size_t i = 0; i < 4; i++)
    thrust_valve_schedule[i] = setting[i];
  enable();
}
