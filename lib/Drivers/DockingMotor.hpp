...
class DockingMotor : public Device {
   public:
    //! Enum as a descriptor for whether the docking motor has been activated or
    //! not
    enum DOCKINGMOTOR_STATE { ON = 0, OFF = 1 };
     ...
     */
    DockingMotor(const std::string &name, unsigned char i1, unsigned char i2, unsigned char dir,
                 unsigned char sleep, unsigned char step);
   ...
   private:
    ...
    //! True if motor in "docked" configuration, false if in "undocked"
    //! configuration.
    bool docked_;
};
}  // namespace Devices
