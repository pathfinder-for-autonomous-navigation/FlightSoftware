...
// Standard device functions
  ...
  /**
   * @brief Runs read over UART buffer to process values sent by Piksi using
   * libsbp
   *
   * @return Returns the number of bytes processed by a call of
   * process_buffer_msg_len() These values coincide with the message size in the
   * piksi documentation.
   *
   * However message size is smaller than the actual number of bytes sent/seen
   * in serial port. Thus the sum of bytes in the serial port != sum of all
   * process_buffer_msg_len() returns.
   */
   ...
   * 5 if the process_buffer loop is unable to deal with all the bytes in
   * READ_ALL_LIMIT microseconds
   */
   ...
   * Let it be known that pos_ecef_flags seems unreliable, not used in
   * PiksiControlTask
   ...
   *
   * @param position A pointer to the std::array of doubles for the baseline
   * position
   */
   ...
   * position
   */
  virtual void get_baseline_ecef(unsigned int *tow,
   ...
   *  @param position A pointer to an std::array of doubles for storing the
   * x,y,z coordinates of the base station. **/
...
// set of mocking methods
  ...
  void set_pos_ecef(const unsigned int tow,
                    const std::array<double, 3> &position,
                    const unsigned char nsats);
  void set_vel_ecef(const unsigned int tow,
                    const std::array<double, 3> &velocity);
  void set_baseline_ecef(const unsigned int tow,
                         const std::array<double, 3> &position);
...
protected:
#ifndef DESKTOP
  HardwareSerial &_serial_port; // This is protected instead of private so that
                                // FakePiksi can access the port variable
#endif
private:
  ...
  static void _gps_time_callback(u16 sender_id, u8 len, u8 msg[],
                                 void *context);
  static void _dops_callback(u16 sender_id, u8 len, u8 msg[], void *context);
  static void _pos_ecef_callback(u16 sender_id, u8 len, u8 msg[],
                                 void *context);
  static void _baseline_ecef_callback(u16 sender_id, u8 len, u8 msg[],
                                      void *context);
  static void _vel_ecef_callback(u16 sender_id, u8 len, u8 msg[],
                                 void *context);
  static void _base_pos_ecef_callback(u16 sender_id, u8 len, u8 msg[],
                                      void *context);
  static void _iar_callback(u16 sender_id, u8 len, u8 msg[], void *context);
  static void _settings_read_resp_callback(u16 sender_id, u8 len, u8 msg[],
                                           void *context);
  static void _startup_callback(u16 sender_id, u8 len, u8 msg[], void *context);
  static void _heartbeat_callback(u16 sender_id, u8 len, u8 msg[],
                                  void *context);
  static void _uart_state_callback(u16 sender_id, u8 len, u8 msg[],
                                   void *context);
  static void _user_data_callback(u16 sender_id, u8 len, u8 msg[],
                                  void *context);
  ...
  unsigned char
      _logbook_size; // How much of the logbook is currently being used.
