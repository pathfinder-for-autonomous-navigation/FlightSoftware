...
Gomspace::Gomspace(const std::string &name,
    ...
    : I2CDevice(name, i2c_wire, i2c_addr, 2000), hk(hk_data),
      gspace_config(config_data), gspace_config2(config2_data) {
  hk_vi = (volatile eps_hk_vi_t *)((volatile unsigned char *)hk + 0);
  hk_out = (volatile eps_hk_out_t *)((volatile unsigned char *)hk_vi +
                                     sizeof(eps_hk_vi_t));
  hk_wdt = (volatile eps_hk_wdt_t *)((volatile unsigned char *)hk_out +
                                     sizeof(eps_hk_out_t));
  hk_basic = (volatile eps_hk_basic_t *)((volatile unsigned char *)hk_wdt +
                                         sizeof(eps_hk_wdt_t));
}
...
bool Gomspace::set_single_output(unsigned char channel, unsigned char value,
                                 short int delay) {
  if (channel > 5)
    return false; // Disallow turning on heater with this command.
  if (value > 1)
    return false; // Precondition check to avoid radiation bit flips
  // All values must be sent in big-endian order!
  delay = __bswap_16(delay);

  unsigned char PORT_BYTE = 0x0A;
  unsigned char command[5] = {PORT_BYTE, channel, value,
...
bool Gomspace::set_pv_volt(unsigned short int voltage1,
                           unsigned short int voltage2,
...
bool Gomspace::config2_get() {
  ...
  if (buffer[1] != 0)
    ...
    // Flip endianness
    gspace_config2->batt_maxvoltage =
        __bswap_16(gspace_config2->batt_maxvoltage);
    gspace_config2->batt_safevoltage =
        __bswap_16(gspace_config2->batt_safevoltage);
    gspace_config2->batt_criticalvoltage =
        __bswap_16(gspace_config2->batt_criticalvoltage);
    gspace_config2->batt_normalvoltage =
        __bswap_16(gspace_config2->batt_normalvoltage);
    return true;
  }
}
...
bool Gomspace::config2_set(const eps_config2_t &c) {
  ...
  config2_struct_ptr->batt_criticalvoltage =
      __bswap_16(config2_struct_ptr->batt_criticalvoltage);
  config2_struct_ptr->batt_maxvoltage =
      __bswap_16(config2_struct_ptr->batt_maxvoltage);
  config2_struct_ptr->batt_safevoltage =
      __bswap_16(config2_struct_ptr->batt_safevoltage);
  config2_struct_ptr->batt_normalvoltage =
      __bswap_16(config2_struct_ptr->batt_normalvoltage);
