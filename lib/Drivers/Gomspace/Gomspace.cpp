...
Gomspace::Gomspace(volatile Gomspace::eps_hk_t *hk_data, 
                   ...
                   i2c_t3 &i2c_wire, unsigned char i2c_addr) : I2CDevice(i2c_wire, i2c_addr, 2000), 
                                                               hk(hk_data),
...
bool Gomspace::set_single_output(unsigned char channel, unsigned char value, short int delay) {
    ...
    unsigned char command[5] = {PORT_BYTE, channel, value, (unsigned char) (delay >> 8), (unsigned char) (delay)};
    i2c_begin_transmission();
    i2c_write(command, 5);
    i2c_end_transmission(I2C_NOSTOP);
...
bool Gomspace::set_pv_volt(unsigned short int voltage1, unsigned short int voltage2, unsigned short int voltage3) {
    // All values must be sent in big-endian order!
    ...
    unsigned char command[7] = {PORT_BYTE, 
        (unsigned char) (voltage1 >> 8), (unsigned char)(voltage1), 
        (unsigned char) (voltage2 >> 8), (unsigned char)(voltage2), 
        (unsigned char) (voltage3 >> 8), (unsigned char)(voltage3)};
    i2c_begin_transmission();
    i2c_write(command, 7);
    i2c_end_transmission(I2C_NOSTOP);
...
bool Gomspace::config_get() {
    ...
    if (buffer[1] != 0) return false;
        ...
        for(unsigned char i = 0; i < 3; i++) { gspace_config->vboost[i] = __bswap_16(gspace_config->vboost[i]); }
        return true;
    }
}
...
bool Gomspace::config_set(const eps_config_t &c) {
    ...
    for(unsigned char i = 0; i < 3; i++) { config_struct_ptr->vboost[i] = __bswap_16(config_struct_ptr->vboost[i]); }
...
bool Gomspace::restore_default_config2() {
    unsigned char PORT_BYTE = 0x15; unsigned char COMMAND_BYTE = 0x02;
