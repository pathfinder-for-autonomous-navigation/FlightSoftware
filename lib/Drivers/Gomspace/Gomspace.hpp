...
namespace Devices {
/**
 * @brief Device class for interacting with the Gomspace battery management system.
 */
class Gomspace : public I2CDevice {
  public:
    ...
    struct __attribute__((packed)) eps_hk_t {
        ...
        short int temp[6];               //! Temperatures [degC] [0 = TEMP1, TEMP2, TEMP3, TEMP4, BP4a, BP4b]
        unsigned char bootcause;             //! Cause of last EPS reset
        unsigned char battmode;              //! Mode for battery [0 = initial, 1 = undervoltage, 2 = safemode, 3 = nominal, 4=full]
    ...
    {
        unsigned int counter_boot; //! Number of EPS reboots
        short int temp[6];       //! Temperatures [degC] [0 = TEMP1, TEMP2, TEMP3, TEMP4, BATT0, BATT1]
        unsigned char bootcause;     //! Cause of last EPS reset
        unsigned char battmode;      //! Mode for battery [0 = initial, 1 = undervoltage, 2 = safemode, 3 = nominal, 4=full]
        unsigned char pptmode;       //! Mode of PPT tracker [1=MPPT, 2=FIXED]
        unsigned short int reserved2;
    ...
    struct __attribute__((packed)) eps_config_t {
        ...
        unsigned short int output_initial_on_delay[8];  //! Output switches: init with these on delays [s]
        unsigned short int output_initial_off_delay[8]; //! Output switches: init with these off delays [s]
    ...
    bool set_pv_volt(unsigned short int voltage1, unsigned short int voltage2, unsigned short int voltage3);
    /** @brief Set power point mode (PPT).
     *  @param mode Which mode to use. See NanoPower documentation for available modes. */
     ...
     *  @param value The value to ping with. The device should send this value back.
