...
static constexpr int OK =
    0; // command was succesfully executed, expected response received
static constexpr int TIMEOUT = 1; // response not received before timeout
static constexpr int BAD_CHECKSUM =
    2; // checksum doesn't match ISU calculated checksum
static constexpr int WRONG_LENGTH =
    3; // message size differs from expected message size
static constexpr int UNEXPECTED_RESPONSE =
    -20; // actual response does not match expected response
static constexpr int WRITE_FAIL =
    -30; // failed to send command (write command to output port)
...
static constexpr int WRONG_FN_ORDER =
    -70; // attempt to execute commands in the wrong order (unexpected order)
 ...
 * of their command
 * Methods that end a command sequence are prefixed with "get_" followed by the
 * name of their command. These methods usually return a status code or retrieve
 * a message.
 *
...
public:
   ...
   */
#ifndef DESKTOP
  QLocate(const std::string &name, HardwareSerial *const port,
