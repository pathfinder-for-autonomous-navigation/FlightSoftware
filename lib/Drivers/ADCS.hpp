...
public:
   ...
   */
  template <typename T>
  void i2c_point_and_read(unsigned char data_register, T *data,
                          std::size_t len);
   ...
   */
  void set_rwa_mode(const unsigned char rwa_mode,
                    const std::array<float, 3> &rwa_cmd);

  /**
   * @brief Set the reaction wheel assembly momentum read exponential filter
   * constant
   *
   * @param rwa_speed_flt momentum read exponential filter, a float from
   * [0.0, 1.0]. The float maps to an eight-bit unsigned integer mapping from
   * [0.0, 1.0]. Defaults to 0xFF.
   */
   ...
   *
   * @param voltage_filter float representing voltage filter value from
   * [0.0, 1.0]. The float maps to an eight-bit unsigned integer mapping from
   * [0.0, 1.0]. Defaults to 0xFF.
   */
   ...
   * in the x, y, and z direction in the body frame.
   */
  void get_rwa(std::array<float, 3> *rwa_momentum_rd,
  ...
  /**
   * @brief Get the imu magnetoruqer, gyroscope and gyroscope temperature
   * reading
   *
   ...
   */
  void get_imu(std::array<float, 3> *mag_rd, std::array<float, 3> *gyr_rd,
               float *gyr_temp_rd);
