...
SystemOutput::SystemOutput(const std::string &n, HardwareSerial &sp)
    : Device(n), _serial_port(sp) {}
...
void SystemOutput::send_propagated_other_position(
    double (&propagated_other_position)[3]) {
  _serial_port.write('o');
  write_double_arr(_serial_port, propagated_other_position);
}

void SystemOutput::send_propagated_other_velocity(
    double (&propagated_other_velocity)[3]) {
