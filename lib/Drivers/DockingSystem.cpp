...
DockingSystem::DockingSystem() : Device("docking_system") {}
...
#ifndef DESKTOP
  return digitalRead(DCDC::dcdc_sph_enable_pin) && digitalRead(motor_sleep_pin);
#else
  return true;
#endif
}
...
void DockingSystem::set_step_angle(float angle) { step_angle = angle; }
...
float DockingSystem::get_step_angle() const { return step_angle; }

void DockingSystem::set_turn_angle(float angle) { steps = angle / step_angle; }

unsigned int DockingSystem::get_steps() const { return steps; }

#ifdef DESKTOP
void DockingSystem::set_dock(bool dock) { is_docked = dock; }
#endif
