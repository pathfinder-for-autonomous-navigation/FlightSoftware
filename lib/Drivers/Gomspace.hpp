...
public:
  static constexpr unsigned char ADDRESS =
      0x02; /**< I2C address of Gomspace device **/
  ...
  struct __attribute__((packed)) eps_hk_vi_t {
    unsigned short int
        vboost[3]; //! Voltage of boost converters [mV] [PV1, PV2, PV3]
    unsigned short int vbatt;     //! Voltage of battery [mV]
    unsigned short int curin[3];  //! Current in [mA]
    unsigned short int cursun;    //! Current from boost converters [mA]
    unsigned short int cursys;    //! Current out of battery [mA]
  ...
  struct __attribute__((packed)) eps_config_t {
    ...
    unsigned char
        battheater_mode;         //! Mode for battheater [0 = Manual, 1 = Auto]
  ...
  Gomspace(const std::string &name, volatile eps_hk_t *hk_data,
           volatile eps_config_t *config_data,
           volatile eps_config2_t *config2_data, i2c_t3 &i2c_wire,
