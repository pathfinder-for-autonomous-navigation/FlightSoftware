...
inline void I2CDevice::i2c_read_from_subaddr(unsigned char subaddr, unsigned char* dest, std::size_t len) {
...
inline void I2CDevice::i2c_write_to_subaddr(unsigned char subaddr, const unsigned char data[], std::size_t len) {
  i2c_begin_transmission();
  i2c_write(subaddr);
  i2c_write(data, len);
  i2c_end_transmission();
...
inline void I2CDevice::i2c_read(T *data, std::size_t len) {
  bool err =
      (this->wire.read((unsigned char *)data, len * sizeof(T)) != len * sizeof(T));
  this->recent_errors = (this->recent_errors || err);
}
