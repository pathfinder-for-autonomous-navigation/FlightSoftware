 ...
 * the access and control of I2C-based hardware peripherals.
 */
...
void I2CDevice::i2c_receive_data(T *data, std::size_t len, i2c_stop s) {
  ...
  for (std::size_t i = 0; i < len * sizeof(T); i++)
    ((unsigned char *)data)[i] = this->i2c_read();
#endif
}
...
inline void I2CDevice::i2c_request_from_subaddr(unsigned char subaddr,
                                                std::size_t len) {
#ifndef DESKTOP
...
inline void I2CDevice::i2c_read_from_subaddr(unsigned char subaddr,
                                             unsigned char *dest,
                                             std::size_t len) {
#ifndef DESKTOP
...
inline void I2CDevice::i2c_write_to_subaddr(unsigned char subaddr,
...
#ifndef DESKTOP
  bool err = (this->wire.read((unsigned char *)data, len * sizeof(T)) !=
              len * sizeof(T));
  this->recent_errors = (this->recent_errors || err);
#endif
}
