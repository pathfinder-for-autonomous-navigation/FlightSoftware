...
void setup() {
    delay(5000);
    DEBUG_init(9600) DEBUG_printlnF("Initializing i2c bus")
    ...
    for (unsigned int i = 0; i < 5; i++) {
        ...
        for (unsigned int j = 0; j < 10; j++) {
            ...
            DEBUG_printlnF("Here again")
                ...
                DEBUG_print(String(val) + ",") delay(200);
        }
        ...
        for (unsigned int j = 0; j < 10; j++) {
            int16_t val;
            analogToDigital.read(ADS1015::CHANNEL::DIFFERENTIAL_0_1, val);
            DEBUG_print(String(val) + ",") delay(400);
        }
        DEBUG_println()
        ...
        for (unsigned int j = 0; j < 10; j++) {
            int16_t val;
            analogToDigital.read(ADS1015::CHANNEL::DIFFERENTIAL_0_1, val);
            DEBUG_print(String(val) + ",") delay(100);
        }
        DEBUG_println()
    }
