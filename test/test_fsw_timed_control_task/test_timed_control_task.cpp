...
public:
  ...
  TestFixture() : registry() {
    clock_manager =
        std::make_unique<ClockManager>(registry, control_cycle_size);

    constexpr unsigned int allocated_starts[2] = {2001, 6001};
    dummy_task_1 =
        std::make_unique<DummyTimedControlTask>(registry, allocated_starts[0]);
    dummy_task_2 =
        std::make_unique<DummyTimedControlTask>(registry, allocated_starts[1]);
  }

  /**
   * @brief Run the control loop, consisting of the Clock Manager and two dummy
   * tasks.
   *
  ...
  sys_time_t execute() {
    ...
    dummy_task_1->execute_on_time(
        clock_manager->get_control_cycle_start_time());
   ...
   * @return sys_time_t
   */
  sys_time_t get_system_time() { return clock_manager->get_system_time(); }
...
void test_task_initialization() { TestFixture tf; }
...
void test_task_execute() {
  ...
  // Stress test the timing guarantee for the control cycle time over many
  // cycles
  t_start = tf.get_system_time();
  ...
  constexpr unsigned long expected_duration =
      TestFixture::control_cycle_ms * 1000 - 2000;
