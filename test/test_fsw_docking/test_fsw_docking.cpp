...
public:
  ...
  TestFixture() : registry() {
    docking_config_cmd_fp =
        registry.create_writable_field<bool>("docksys.config_cmd");
    docking_config_cmd_fp->set(false);

    docking_controller =
        std::make_unique<DockingController>(registry, 0, docksys);

    docked_fp = registry.find_readable_field_t<bool>("docksys.docked");
    dock_config_fp =
        registry.find_readable_field_t<bool>("docksys.dock_config");
...
void test_task_execute() {
  TestFixture tf;
  // affirm that if the motor is in the docking config and the mission manager
  // doesn't want to undock, then the motor should remain in the docking config
