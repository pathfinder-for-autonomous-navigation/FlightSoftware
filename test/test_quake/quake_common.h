...
typedef struct sbdix_r {
  ...
  int MO_status;
  // Sequence number that Quake increments when message successfully sent from
  // Quake to Gateway
  int MOMSN;
  // Disposition of mobile terminated transaction
  int MT_status; // 0 -> no message to receive, 1 -> msg received, 2-> error
  // Assigned by GSS when forwarding message to Quake (indeterminate value if
  // MT_status = 0)
  int MTMSN;
...
/**
 * [callback] is a function pointer to a member function of QLocate (i.e.
 * get_sbdwb) This function waits [DEFAULT_CTRL_LENGTH] everytime [callback]
 * returns with PORT_UNAVAILABLE. [fnName] is the name of the function that
 * callback points to.
 */
