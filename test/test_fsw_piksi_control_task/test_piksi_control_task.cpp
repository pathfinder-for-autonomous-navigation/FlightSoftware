...
#define assert_piksi_mode(x)                                                   \
  {                                                                            \
    TEST_ASSERT_EQUAL(x,                                                       \
                      static_cast<piksi_mode_t>(tf.currentState_fp->get()));   \
  }
...
// Create a TestFixture instance of PiksiController with pointers to statefields
  ...
  TestFixture() : registry(), piksi("piksi", Serial4) {
    ...
    baseline_fp =
        registry.find_readable_field_t<d_vector_t>("piksi.baseline_pos");
  ...
  void execute() { piksi_task->execute(); }
...
float mag_2(const std::array<double, 3> input) {
  return (float)(input[0] * input[0] + input[1] * input[1] +
                 input[2] * input[2]);
}
