...
#define assert_fn_num(x)                                                       \
  {                                                                            \
    TEST_ASSERT_EQUAL(                                                         \
        x, tf.quake_manager->dbg_get_qct().get_current_fn_number());           \
  }
...
char *snap1 = (char *)"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
                      "AAAAAAAAAAAAAA"
...
public:
  ...
  // Quake has no output state fields since it is created after downlink
  // producer
  ...
  TestFixture(unsigned int radio_mode, int qct_state) : registry() {
    ...
    snapshot_size_fp = registry.create_internal_field<unsigned int>(
        "downlink_producer.snap_size");
    radio_mo_packet_fp =
        registry.create_internal_field<char *>("downlink_producer.mo_ptr");
    radio_mt_packet_fp =
        registry.create_internal_field<char *>("uplink_consumer.mt_ptr");
    radio_err_fp = registry.create_readable_field<int>(
        "downlink_producer.radio_err_ptr", -90, 10);
    radio_mt_ready_fp =
        registry.create_internal_field<bool>("uplink_producer.mt_ready");
    ...
    if (qct_state !=
        -1) // If qct_state == -1, then expect use the default initialization
    {
...
void test_trans_unexpected() {
  test_rwt_unexpected(SBDIX,
                      static_cast<unsigned int>(radio_mode_t::transceive));
}
...
void test_wait_no_more_cycles() {
  ...
  TEST_ASSERT_EQUAL(initCycles + tf.quake_manager->max_wait_cycles,
                    tf.cycle_no_fp->get());
  tf.step();
  assert_qct(SBDWB);
  assert_fn_num(0);
...
void test_rwc_no_more_cycles(int qct_state, unsigned int radio_mode) {
  ...
  tf.cycle_no_fp->set(tf.cycle_no_fp->get() +
                      tf.quake_manager->max_transceive_cycles);
  TEST_ASSERT_EQUAL(initCycles + tf.quake_manager->max_transceive_cycles,
                    tf.cycle_no_fp->get());
  tf.realSteps();
  tf.realSteps();
  // then expect transition to WAIT and SBDWB, CONFIG, SBDRB should set the
  // error flag
  assert_qct(IDLE);
  assert_radio_state(radio_mode_t::wait);
  assert_fn_num(0);
  if (static_cast<unsigned int>(radio_mode) !=
      static_cast<unsigned int>(radio_mode_t::transceive)) {
    TEST_ASSERT_TRUE(tf.quake_manager->dbg_get_unexpected_flag());
  }
}
...
void test_config_no_more_cycles() {
  test_rwc_no_more_cycles(CONFIG,
                          static_cast<unsigned int>(radio_mode_t::config));
}
...
void test_transceive_ok_no_network_timed_out() {
  ...
  TestFixture tf(static_cast<unsigned int>(radio_mode_t::transceive), SBDIX);
  tf.cycle_no_fp->set(tf.cycle_no_fp->get() +
                      tf.quake_manager->max_transceive_cycles - 1);
  tf.step(); // 0
  tf.quake_manager->dbg_get_qct().dbg_get_quake().sbdix_r[0] = 32;
  tf.step(); // 1
  // then expect transition to WAIT
...
void test_oldcycles_do_not_change() {
  ...
  TEST_ASSERT_EQUAL(tf.cycle_no_fp->get(),
                    tf.quake_manager->dbg_get_last_checkin());
}
...
char *snap2 = (char *)"11111111111111111111111111111111111111111111111111111111"
                      "11111111111111"
...
void test_write_load_message() {
  ...
  check_buf_bytes(
      ...
      tf.quake_manager->dbg_get_qct().dbg_get_MO_msg(),
      tf.quake_manager->dbg_get_qct().dbg_get_MO_len());
  tf.step();
}
...
void test_update_mo_same_snap() {
  ...
  TEST_ASSERT_EQUAL_STRING(snap1,
                           tf.quake_manager->dbg_get_qct().dbg_get_MO_msg());
  tf.radio_mo_packet_fp->set(snap2);
  tf.realSteps(); // 1
  tf.quake_manager->dbg_get_qct().dbg_get_quake().sbdix_r[2] =
      0;          // have comms but no msg
  tf.realSteps(); // 2
  tf.radio_mo_packet_fp->set(snap2);
  // Execute SBDIX
  TEST_ASSERT_EQUAL_STRING(snap1,
  ...
  // Return to SBDWB
  tf.realSteps(); // sbdwb 0
  // then expect the second piece of the snapshot should be loaded on the next
  // write
  check_buf_bytes(
...
void test_update_mo_reset_idx() {
  ...
  tf.realSteps(2); // 1, 2
  tf.radio_mo_packet_fp->set(snap2);
  // then expect mo_idx should be reset to 0 but mo_buffer should still be the
  // same
  TEST_ASSERT_EQUAL(0, tf.quake_manager->dbg_get_mo_idx());
...
void test_update_mo_load_new_snap() {
  ...
  TestFixture tf(static_cast<unsigned int>(radio_mode_t::write), SBDWB);
  // Read Snap 1 (which is already loaded in the constructor of the test
  // fixture)
  tf.realSteps(3);
  tf.radio_mo_packet_fp->set(snap2);
  tf.realSteps(2);
  ...
  TEST_ASSERT_EQUAL_STRING(snap2,
                           tf.quake_manager->dbg_get_qct().dbg_get_MO_msg());
  check_buf_bytes(
...
#ifdef DESKTOP
int main() { return test_mission_manager(); }
#else
#include <Arduino.h>
void setup() {
  delay(2000);
...
/*
parse to test
cat test/test_quake_manager//test_quake_manager.cpp | grep "void test_" | sed
's/^void \(.*\)$/\1/' | sed 's/()/);/g'| sed -e 's/^/RUN_TEST(/'
*/