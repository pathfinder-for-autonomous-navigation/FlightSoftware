...
void setup() {
    DEBUG_init(9600) Wire2.begin(I2C_MASTER, 0x00, I2C_PINS_3_4, I2C_PULLUP_EXT, 400000);
    ...
    if (gyro.reset()) {
        ...
        DEBUG_printlnF("Gyroscope initialization failed") while (1);
    }
}
...
void loop() {
    ...
    if (gyro.read()) {
        if (millis() - 2000 > time_stamp) {
            time_stamp = millis();
            DEBUG_println("omega = " + String(gyro.get_omega_x()) + "," +
