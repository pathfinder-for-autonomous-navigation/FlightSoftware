/** @file ADCS.hpp
 * @author Kyle Krol, Shihao Cao
 * @date Spring 2018, Fall 2019
 * @brief Contains implementation for device interface to ADCS system.
 */

#include "ADCS.hpp"

#include <adcs/constants.hpp>
#include <adcs/state_registers.hpp>

#include <cstring>

using namespace Devices;

#ifndef DESKTOP
ADCS::ADCS(i2c_t3 &i2c_wire, uint8_t address)
    : I2CDevice("adcs", i2c_wire, address, 1000) {}
#else
ADCS::ADCS()
    : I2CDevice("adcs", 0) {}
#endif

bool ADCS::i2c_ping() {
    uint8_t temp = 0;
    get_who_am_i(&temp); 
    return temp==WHO_AM_I_EXPECTED;
}

template <typename T>
void ADCS::i2c_point_and_read(uint8_t data_register, T* data, std::size_t len) {
    set_read_ptr(data_register);
    i2c_request_from(len);
    i2c_read(data, len);
}

// static float fp(int8_t si, float min, float max) {
//   return min + (((float) si) + 128.0f) * (max - min) / 255.0f;
// }

static float fp(uint8_t ui, float min, float max) {
  return min + ((float) ui) * (max - min) / 255.0f;
}

// static float fp(int16_t si, float min, float max) {
//   return min + (((float) si) + 32768.0f) * (max - min) / 65535.0f;
// }

static float fp(uint16_t ui, float min, float max) {
  return min + ((float) ui) * (max - min) / 65535.0f;
}

static uint8_t uc(float f, float min, float max) {
  return (uint8_t)(255.0f * (f - min) / (max - min));
}

// static int8_t sc(float f, float min, float max) {
//   return (int8_t)(255.0f * (f - min) / (max - min) - 128.0f);
// }

static uint16_t us(float f, float min, float max) {
  return (uint16_t)(65535.0f * (f - min) / (max - min));
}

// static int16_t ss(float f, float min, float max) {
//   return (int16_t)(65535.0f * (f - min) / (max - min) - 32768.0f);
// }

void ADCS::set_mode(uint8_t mode) {
    i2c_write_to_subaddr(adcs::ADCS_MODE, mode);
}

void ADCS::set_read_ptr(uint8_t read_ptr){
    i2c_write_to_subaddr(adcs::READ_POINTER, read_ptr);

}

void ADCS::set_rwa_mode(uint8_t rwa_mode, const std::array<float,3>& rwa_cmd){
    i2c_write_to_subaddr(adcs::RWA_MODE, rwa_mode);

    uint8_t cmd[6];
    for(uint8_t i = 0;i<3;i++){
        uint16_t comp = 0;
        if(rwa_mode == 1)
            comp = us(rwa_cmd[i],adcs::rwa::min_speed_command,adcs::rwa::max_speed_command);
        else if(rwa_mode == 2)
            comp = us(rwa_cmd[i],adcs::rwa::min_torque,adcs::rwa::max_torque);
        cmd[2*i] = comp;
        cmd[2*i+1] = comp >> 8;
    }
    i2c_write_to_subaddr(adcs::RWA_COMMAND,cmd,6);
}

void ADCS::set_rwa_speed_filter(float mom_filter){
    uint8_t comp = uc(mom_filter,0.0f,1.0f);
    i2c_write_to_subaddr(adcs::RWA_SPEED_FILTER, comp);
}

void ADCS::set_ramp_filter(float ramp_filter){
    uint8_t comp = uc(ramp_filter,0.0f,1.0f);
    i2c_write_to_subaddr(adcs::RWA_RAMP_FILTER, comp);
}

void ADCS::set_mtr_mode(uint8_t mtr_mode){
    i2c_write_to_subaddr(adcs::MTR_MODE, mtr_mode);
}

void ADCS::set_mtr_cmd(const std::array<float, 3> &mtr_cmd){
    uint8_t cmd[6];
    for(uint8_t i = 0;i<3;i++){
        uint16_t comp = us(mtr_cmd[i],adcs::mtr::min_moment,adcs::mtr::max_moment);
        cmd[2*i] = comp;
        cmd[2*i+1] = comp >> 8; 
    }
    i2c_write_to_subaddr(adcs::MTR_COMMAND,cmd,6);
}

void ADCS::set_mtr_limit(float mtr_limit){
    uint8_t cmd[2];
    uint16_t comp = us(mtr_limit,adcs::mtr::min_moment,adcs::mtr::max_moment);
    cmd[0] = comp;
    cmd[1] = comp >> 8; 
    i2c_write_to_subaddr(adcs::MTR_LIMIT, cmd, 2);
}

void ADCS::set_ssa_mode(uint8_t ssa_mode) {
    i2c_write_to_subaddr(adcs::SSA_MODE, ssa_mode);
}

void ADCS::set_ssa_voltage_filter(float voltage_filter) {
    uint8_t comp = uc(voltage_filter,0.0f,1.0f);
    i2c_write_to_subaddr(adcs::SSA_VOLTAGE_FILTER, comp);
}

void ADCS::set_imu_mode(uint8_t mode){
    i2c_write_to_subaddr(adcs::IMU_MODE, mode);
}

void ADCS::set_imu_mag_filter(float mag_filter){
    uint8_t comp = uc(mag_filter,0.0f,1.0f);
    i2c_write_to_subaddr(adcs::IMU_MAG_FILTER, comp);
}

void ADCS::set_imu_gyr_filter(float gyr_filter){
    uint8_t comp = uc(gyr_filter,0.0f,1.0f);
    i2c_write_to_subaddr(adcs::IMU_GYR_FILTER, comp);
}

void ADCS::set_imu_gyr_temp_filter(float temp_filter){
    uint8_t comp = uc(temp_filter,0.0f,1.0f);
    i2c_write_to_subaddr(adcs::IMU_GYR_TEMP_FILTER, comp);
}

void float_decomp(float input, uint8_t* temp){
    //turns the input float into 4 chars
    *(float*)(temp) = input;
}

void ADCS::set_imu_gyr_temp_kp(float kp){
    uint8_t cmd[4];
    float_decomp(kp, cmd);
    i2c_write_to_subaddr(adcs::IMU_GYR_TEMP_KP,cmd,4);
}

void ADCS::set_imu_gyr_temp_ki(float ki){
    uint8_t cmd[4];
    float_decomp(ki, cmd);
    i2c_write_to_subaddr(adcs::IMU_GYR_TEMP_KI,cmd,4);
}

void ADCS::set_imu_gyr_temp_kd(float kd){
    uint8_t cmd[4];
    float_decomp(kd, cmd);
    i2c_write_to_subaddr(adcs::IMU_GYR_TEMP_KD,cmd,4);
}

void ADCS::set_imu_gyr_temp_desired(float desired){
    uint8_t cmd = uc(desired,adcs::imu::min_eq_temp,adcs::imu::max_eq_temp);
    i2c_write_to_subaddr(adcs::IMU_GYR_TEMP_DESIRED,cmd);
}

void ADCS::set_havt_reset(const std::bitset<adcs::havt::max_devices>& table){
    //4 because 32/8 = 4
    uint8_t cmd[4];

    uint32_t encoded = (uint32_t)table.to_ulong();

    //dissassemble uint32_t into 4 chars
    uint8_t * encoded_ptr = (uint8_t *)(&encoded);
    for (uint32_t i = 0; i < 4; i++){
        cmd[i] = encoded_ptr[i];
    }

    i2c_write_to_subaddr(adcs::HAVT_COMMAND_RESET, cmd, 4);
}

void ADCS::set_havt_disable(const std::bitset<adcs::havt::max_devices>& table){
    //4 because 32/8 = 4
    uint8_t cmd[4];

    uint32_t encoded = (uint32_t)table.to_ulong();

    //dissassemble uint32_t into 4 chars
    uint8_t * encoded_ptr = (uint8_t *)(&encoded);
    for (uint32_t i = 0; i < 4; i++){
        cmd[i] = encoded_ptr[i];
    }

    i2c_write_to_subaddr(adcs::HAVT_COMMAND_DISABLE, cmd, 4);
}

void ADCS::get_who_am_i(uint8_t* who_am_i) {
    i2c_point_and_read(adcs::WHO_AM_I, who_am_i, 1);
}

void ADCS::get_rwa(std::array<float, 3>* rwa_speed_rd, std::array<float, 3>* rwa_ramp_rd) {
    uint8_t readin[12];
    std::memset(readin, 0, sizeof(readin));

    #ifdef UNIT_TEST
    for(uint8_t i = 0;i<12;i++){
        readin[i] = 255;
    }
    #else
    i2c_point_and_read(adcs::RWA_SPEED_RD, readin, 12);
    #endif

    for(uint8_t i=0;i<3;i++){
        uint16_t a = readin[2*i+1] << 8;
        uint16_t b = 0xFF & readin[2*i];
        uint16_t c = a | b;
        (*rwa_speed_rd)[i] = fp(c,adcs::rwa::min_speed_read,adcs::rwa::max_speed_read);
    }
    for(uint8_t i=0;i<3;i++){
        uint16_t a = readin[2*i+1+6] << 8;
        uint16_t b = 0xFF & readin[2*i+6];
        uint16_t c = a | b;
        (*rwa_ramp_rd)[i] = fp(c,adcs::rwa::min_torque,adcs::rwa::max_torque);
    }
}

void ADCS::get_imu(std::array<float,3>* mag_rd,std::array<float,3>* gyr_rd,float* gyr_temp_rd){
    uint8_t readin[14];
    std::memset(readin, 0, sizeof(readin));

    #ifdef UNIT_TEST
    for(uint8_t i = 0;i<14;i++){
        readin[i] = 255;
    }
    #else
    i2c_point_and_read(adcs::IMU_MAG_READ, readin, 14);
    #endif

    for(uint8_t i=0;i<3;i++){
        uint16_t a = readin[2*i+1] << 8;
        uint16_t b = 0xFF & readin[2*i];
        uint16_t c = a | b;        
        (*mag_rd)[i] = fp(c,adcs::imu::min_rd_mag,adcs::imu::max_rd_mag);
    }

    for(uint8_t i=0;i<3;i++){
        uint16_t a = readin[2*i+1+6] << 8;
        uint16_t b = 0xFF & readin[2*i+6];
        uint16_t c = a | b;
        (*gyr_rd)[i] = fp(c,adcs::imu::min_rd_omega,adcs::imu::max_rd_omega);
    } 

    uint16_t c = (((uint16_t)readin[13]) << 8) | (0xFF & readin[12]);
    *gyr_temp_rd = fp(c,adcs::imu::min_rd_temp,adcs::imu::max_rd_temp);
}

void ADCS::get_ssa_mode(uint8_t* a) {
    #ifdef UNIT_TEST
    //acceleration control mode, mocking output
    *a = mock_ssa_mode;
    #else
    i2c_point_and_read(adcs::SSA_MODE, a, 1);
    #endif
}

void ADCS::get_ssa_vector(std::array<float, 3>* ssa_sun_vec) {
    uint8_t readin[6];
    std::memset(readin, 0, sizeof(readin));

    #ifdef UNIT_TEST
    for(uint8_t i = 0;i<6;i++){
        readin[i] = 255;
    }
    #else
    i2c_point_and_read(adcs::SSA_SUN_VECTOR, readin,6);
    #endif

    for(uint8_t i=0;i<3;i++){
        uint16_t c = (((uint16_t)readin[2*i+1]) << 8) | (0xFF & readin[2*i]);

        (*ssa_sun_vec)[i] = fp(c,-1.0f,1.0f);
    }

}

void ADCS::get_ssa_voltage(std::array<float, adcs::ssa::num_sun_sensors>* voltages){
    uint8_t temp[adcs::ssa::num_sun_sensors];
    std::memset(temp, 0, sizeof(temp));

    #ifdef UNIT_TEST
    for(uint8_t i = 0;i<adcs::ssa::num_sun_sensors;i++){
        temp[i] = 255;
    }
    #else
    i2c_point_and_read(adcs::SSA_VOLTAGE_READ,temp,adcs::ssa::num_sun_sensors);
    #endif
    
    for(uint8_t i = 0;i<adcs::ssa::num_sun_sensors;i++){
        (*voltages)[i] = fp(temp[i], adcs::ssa::min_voltage_rd, adcs::ssa::max_voltage_rd);
    }
}

void ADCS::get_havt(std::bitset<adcs::havt::max_devices>* havt_table){
    // mocking return
    #ifdef UNIT_TEST
    (*havt_table) = mock_havt_read;
    return;
    #endif
    
    //4 because 32/8 = 4
    uint8_t temp[4];
    std::memset(temp, 0, sizeof(temp));
    i2c_point_and_read(adcs::HAVT_READ,temp, 4);

    uint32_t encoded;
    
    //assemble chars into an int
    uint8_t * encoded_ptr = (uint8_t *)(&encoded);
    for (uint32_t i = 0; i < 4; i++){
        encoded_ptr[i] = temp[i];
    }

    (*havt_table) = std::bitset<adcs::havt::max_devices>(encoded);
}

#ifdef UNIT_TEST
void ADCS::set_mock_havt_read(const std::bitset<adcs::havt::max_devices>& havt_input){
    mock_havt_read = havt_input;
}

void ADCS::set_mock_ssa_mode(uint8_t ssa_mode) {
    mock_ssa_mode = ssa_mode;
}

void ADCS::set_mock_adcs_functional(bool functional) {
    adcs_functionality = functional;
}
#endif