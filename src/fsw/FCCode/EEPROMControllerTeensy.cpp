...
void EEPROMController::read_EEPROM(){
  for (unsigned int i = 0; i<_registry.eeprom_saved_fields.size(); i++){
    _registry.eeprom_saved_fields.at(i)->set_from_eeprom(EEPROM.read(addresses.at(i)));
  }
}

void EEPROMController::update_EEPROM(unsigned int position){
  EEPROM.put(addresses.at(position), _registry.eeprom_saved_fields.at(position)->get_eeprom_repr());
}
