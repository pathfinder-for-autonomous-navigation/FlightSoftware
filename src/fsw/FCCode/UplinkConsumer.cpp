...
UplinkConsumer::UplinkConsumer(StateFieldRegistry& _registry, uint32_t offset) :
    TimedControlTask<void>(_registry, "uplink_ct", offset), Uplink(_registry)
{
    radio_mt_packet_len_fp = find_internal_field<size_t>("uplink.len");
    radio_mt_packet_fp = find_internal_field<char*>("uplink.ptr");
}
