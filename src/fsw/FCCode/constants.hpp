...
namespace PAN {
    ...
    // other constants are just informational.
        ...
        #else
            TRACKED_CONSTANT(constexpr unsigned int, control_cycle_time, 170000)
        #endif
    #elif defined(FLIGHT) || defined(UNIT_TEST)
        TRACKED_CONSTANT(constexpr unsigned int, control_cycle_time_ms, 120)
        TRACKED_CONSTANT(constexpr unsigned int, control_cycle_time_us, 120000)
        TRACKED_CONSTANT(constexpr unsigned int, control_cycle_time, 120000)
    #endif

    TRACKED_CONSTANT(constexpr unsigned int, one_day_ccno, 24 * 60 * 60 * 1000 / control_cycle_time_ms)
}
