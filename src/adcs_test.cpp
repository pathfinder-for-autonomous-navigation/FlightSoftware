...
bool test_get_imu() {
  ...
  return (comp_float_arr(mag_rd, mag_state, 0.0001f) &&
          comp_float_arr(gyr_rd, gyr_state, 0.001f) &&
...
bool test_get_rwa() {
  ...
  std::array<float, 3> rwa_momentum_state = {0.004f, 0.005f,
                                             -0.006f}; // Momentum read
...
bool test_get_ssa_voltage() {
  ...
  // arbitrary test values
  std::array<float, 20> reference = {1.0f, 2.0f, 3.0f, 0.0f, 0.0f, 1.0f, 2.0f,
                                     3.0f, 0.0f, 0.0f, 1.0f, 2.0f, 3.0f, 0.0f,
                                     0.0f, 1.0f, 2.0f, 3.0f, 0.0f, 0.0f};

  return comp_float_arr(reference, temp, 0.02f);
}

bool test_everything() {
  return adcs.is_functional() && adcs.i2c_ping() &&

         test_set_mode() && test_set_rwa_mode() &&
         test_set_rwa_momentum_filter() && test_set_rwa_ramp_filter() &&
         test_set_mtr_mode() && test_set_mtr_command() &&
         test_set_mtr_limit() && test_getset_ssa_mode() &&
         test_set_ssa_voltage_filter() && test_set_imu_mode() &&
         test_set_imu_filters() &&

         test_get_who_am_i() && test_get_ssa_vector() &&
         test_get_ssa_voltage() && test_get_rwa() && test_get_imu();
}
