...
static void endian_write(T t) {
  uint8_t *ptr = static_cast<uint8_t *>(&t);
  // if (registers.endianess == Endianness::BIG)
  //   for (uint32_t i = sizeof(T) - 1; i >= 0; i--) umb::wire->write(ptr[i]);
  // else
  // ^^^ Endianess option was removed
    for (uint32_t i = 0; i < sizeof(T); i++) umb::wire->write(ptr[i]);
}
...
static void endian_write(T const (&t_arr)[N]) {
  for (uint32_t i = 0; i < N; i++) endian_write(t_arr[i]);
}
...
static T endian_read() {
  ...
  uint8_t *ptr = static_cast<uint8_t *>(&t);
  // if (registers.endianess == Endianness::BIG)
  //   for (uint32_t i = sizeof(T) - 1; i >= 0; i--) ptr[i] = umb::wire->read();
  // else
  // ^^^ Endianess option was removed
    for (uint32_t i = 0; i < sizeof(T); i++) ptr[i] = umb::wire->read();
  return t;
}
...
static void endian_read(T (&t_arr)[N]) {
  for (uint32_t i = 0; i < N; i++) t_arr[i] = endian_read<T>();
}
...
static void copy_to(T const (&t_src)[N], T volatile (&t_dst)[N]) {
  for (uint32_t i = 0; i < N; i++) t_dst[i] = t_src[i];
}
...
void on_i2c_recieve(uint32_t bytes) {
  ...
  uint8_t address = endian_read<uint8_t>();

  switch (address) {

    // case Register::ENDIANNESS: {
    //   if (umb::wire->available() < 1) break;
    //   registers.endianess = endian_read<uint8_t>();
    ...
    case Register::ADCS_MODE: {
      if (umb::wire->available() < 1) break;
      registers.mode = endian_read<uint8_t>();
    ...
    case Register::READ_POINTER: {
      if (umb::wire->available() < 1) break;
      registers.read_ptr = endian_read<uint8_t>();
    ...
    case Register::RWA_MODE: {
      if (umb::wire->available() < 1) break;
      registers.rwa.mode = endian_read<uint8_t>();
    ...
    case Register::RWA_SPEED_FILTER: {
      if (umb::wire->available() < 1) break;
      registers.rwa.momentum_flt = utl::fp(endian_read<uint8_t>(), 0.0f, 1.0f);
    ...
    case Register::RWA_RAMP_FILTER: {
      if (umb::wire->available() < 1) break;
      registers.rwa.ramp_flt = utl::fp(endian_read<uint8_t>(), 0.0f, 1.0f);
    ...
    case Register::MTR_MODE: {
      if (umb::wire->available() < 1) break;
      registers.mtr.mode = endian_read<uint8_t>();
    ...
    case Register::MTR_LIMIT: {
      ...
      registers.mtr.moment_limit = utl::fp(endian_read<uint16_t>(), mtr::min_moment, mtr::max_moment);
      registers.mtr.cmd_flg = CMDFlag::UPDATED;
    ...
    case Register::SSA_MODE: {
      ...
      if (registers.ssa.mode == SSAMode::SSA_COMPLETE ||
        ...
        registers.ssa.mode = endian_read<uint8_t>();
      }
    ...
    case Register::SSA_VOLTAGE_FILTER: {
      if (umb::wire->available() < 1) break;
      registers.ssa.voltage_flt = utl::fp(endian_read<uint8_t>(), 0.0f, 1.0f);
    ...
    case Register::IMU_MODE: {
      if (umb::wire->available() < 1) break;
      registers.imu.mode = endian_read<uint8_t>();
    ...
    case Register::IMU_MAG_FILTER: {
      if (umb::wire->available() < 1) break;
      registers.imu.mag_flt = utl::fp(endian_read<uint8_t>(), 0.0f, 1.0f);
    ...
    case Register::IMU_GYR_FILTER: {
      if (umb::wire->available() < 1) break;
      registers.imu.gyr_flt = utl::fp(endian_read<uint8_t>(), 0.0f, 1.0f);
    ...
    case Register::IMU_GYR_TEMP_FILTER: {
      if (umb::wire->available() < 1) break;
      registers.imu.gyr_temp_flt = utl::fp(endian_read<uint8_t>(), 0.0f, 1.0f);
    ...
    case Register::IMU_GYR_TEMP_DESIRED: {
      if (umb::wire->available() < 1) break;
      registers.imu.gyr_desired_temp = utl::fp(endian_read<uint8_t>(), imu::min_eq_temp, imu::max_eq_temp);
    ...
    case Register::HAVT_COMMAND_RESET: {
      if (umb::wire->available() < 1) break;
      registers.havt.cmd_reset_table = endian_read<uint32_t>();
    ...
    case Register::HAVT_COMMAND_DISABLE: {
      if (umb::wire->available() < 1) break;
      registers.havt.cmd_disable_table = endian_read<uint32_t>();
...
void on_i2c_request() {
  uint8_t address = registers.read_ptr;
  ...
  switch (address) {
    ...
    case Register::HAVT_READ: {
      //table is stored as an uint32_t, so merely write out to i2c
