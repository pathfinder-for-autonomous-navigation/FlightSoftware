...
DockingController::DockingController(StateFieldRegistry &registry,
    ...
    : TimedControlTask<void>(registry, offset), docksys(_docksys), docked_sr(),
      docked_f("docksys.docked", docked_sr), dock_config_sr(),
      dock_config_f("docksys.dock_config", dock_config_sr), is_turning_sr(),
  ...
  docking_config_cmd_fp =
      find_writable_field<bool>("docksys.config_cmd", __FILE__, __LINE__);
...
void DockingController::execute() {
  ...
  if (docking_config_cmd_fp->get()) {
    if (!dock_config_f.get()) {
      // prevents execute() from repeating start dock and setting turning angle
      // to 180
      if (!is_turning_f.get()) {
        docksys.start_dock();
