...
PiksiControlTask::PiksiControlTask(StateFieldRegistry &registry,
    ...
    : TimedControlTask<void>(registry, offset), piksi(_piksi),
      pos_sr(0, 100000, 100), pos_f("piksi.pos", pos_sr),
      vel_sr(0, 100000, 100), vel_f("piksi.vel", vel_sr),
      baseline_pos_sr(0, 100000, 100),
      baseline_pos_f("piksi.baseline_pos", baseline_pos_sr),
      current_state_sr(0, 4, 2),
      current_state_f("piksi.state", current_state_sr), time_sr(),
...
void PiksiControlTask::execute() {
  ...
  if (read_out == 5) {
    current_state_f.set(
        static_cast<unsigned int>(piksi_mode_t::time_limit_error));
    return;
  }
    ...
    if (read_out == 1) {
      ...
      if (baseline_flag == 1) {
        ...
        current_state_f.set(
            static_cast<unsigned int>(piksi_mode_t::data_error));
        return;
      }
    }
