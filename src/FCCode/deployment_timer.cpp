...
void exit_deployment_timer() {
    ...
    dbg.println("Notifying all processes waiting on deployment timer that timer has been completed.");
    chSysLock();
...
void deployment_timer_function(void *arg) {
    ...
    chMtxUnlock(&eeprom_lock);
    if (deployed) {
        dbg.println("Deployment hold time constraint has been met. Skipping deployment timer.");
        exit_deployment_timer();
    }
    else dbg.println("Deployment hold time constraint has not been met. Initializing deployment timer.");

    dbg.println("Deployment timer has been started.");
    ...
    while(time_elapsed < DEPLOYMENT_LENGTH) {
        ...
        dbg.printf("Time remaining until deployment wait completed: %d", DEPLOYMENT_LENGTH - time_elapsed);
        chThdSleepSeconds(1);
        time_elapsed++;
    }

    dbg.println("Deployment timer completed! Writing achievement to permanent memory.");
    exit_deployment_timer();
}