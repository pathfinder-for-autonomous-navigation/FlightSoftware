...
void exit_deployment_timer() {
    ...
    dbg.println(debug_console::severity::INFO, "Notifying all processes waiting on deployment timer that timer has been completed.");
    chSysLock();
...
void deployment_timer_function(void *arg) {
    ...
    chMtxUnlock(&eeprom_lock);
    if (deployed) {
        dbg.println(debug_console::severity::NOTICE, "Deployment hold time constraint has been met. Skipping deployment timer.");
        exit_deployment_timer();
    }
    else dbg.println(debug_console::severity::NOTICE, "Deployment hold time constraint has not been met. Initializing deployment timer.");

    dbg.println(debug_console::severity::NOTICE, "Deployment timer has been started.");
    ...
    while(time_elapsed < DEPLOYMENT_LENGTH) {
        ...
        dbg.printf(debug_console::severity::NOTICE, "Time remaining until deployment wait completed: %d", DEPLOYMENT_LENGTH - time_elapsed);
        chThdSleepSeconds(1);
        time_elapsed++;
    }

    dbg.println(debug_console::severity::NOTICE, "Deployment timer completed! Writing achievement to permanent memory.");
    exit_deployment_timer();
}