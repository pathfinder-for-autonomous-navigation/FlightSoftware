...
QuakeManager::QuakeManager(StateFieldRegistry &registry, unsigned int offset)
    : TimedControlTask<bool>(registry, offset),
      radio_mode_f(radio_mode_t::config), qct(registry), mo_idx(0),
      unexpected_flag(false) {
  // Retrieve fields from registry
  control_cycle_count_fp =
      find_readable_field<unsigned int>("pan.cycle_no", __FILE__, __LINE__);
  snapshot_size_fp = find_internal_field<unsigned int>(
      "downlink_producer.snap_size", __FILE__, __LINE__);
  radio_mo_packet_fp = find_internal_field<char *>("downlink_producer.mo_ptr",
                                                   __FILE__, __LINE__);
  radio_mt_packet_fp =
      find_internal_field<char *>("uplink_consumer.mt_ptr", __FILE__, __LINE__);
  radio_err_fp = find_readable_field<int>("downlink_producer.radio_err_ptr",
                                          __FILE__, __LINE__);
  radio_mt_ready_fp =
      find_internal_field<bool>("uplink_producer.mt_ready", __FILE__, __LINE__);
...
QuakeManager::~QuakeManager() { delete[] mo_buffer_copy; }
...
bool QuakeManager::execute() {
  printf(debug_severity::info, "[Quake Info] Executing Quake Manager \
        current radio_state %d, current control task state %d",
         static_cast<unsigned int>(radio_mode_f), qct.get_current_state());
  switch (radio_mode_f) {
...
bool QuakeManager::dispatch_config() {
  ...
  if (unexpected_flag)
    return transition_radio_state(radio_mode_t::wait);
...
bool QuakeManager::write_to_error(int err_code) {
  ...
  printf(
      ...
      static_cast<unsigned int>(radio_mode_f), qct.get_current_state(),
      qct.get_current_fn_number(), (error));
...
bool QuakeManager::transition_radio_state(radio_mode_t new_state) {
  printf(debug_severity::info,
         ...
         static_cast<unsigned int>(radio_mode_f), new_state);
  bool bOk = true;
  switch (new_state) {
  case radio_mode_t::wait:
  ...
  default:
    printf(debug_severity::error,
           "In transition_radio_state:: Radio state not defined: %d",
           static_cast<unsigned int>(radio_mode_f));
  }
  ...
  if (!bOk) // Sanity check
    printf(debug_severity::error, "Invalid state transition from %d to %d",
           qct.get_current_state(), static_cast<unsigned int>(radio_mode_f));
  return bOk;
}
