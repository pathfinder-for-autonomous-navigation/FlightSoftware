...
void debug_function(void *arg) {
  ...
  while (true) {
    ...
    dbg.printf(debug_severity::DEBUG, "Propulsion: %d",
               chUnusedThreadStack(
                   RTOSTasks::propulsion_controller_workingArea,
                   sizeof(RTOSTasks::propulsion_controller_workingArea)));
    dbg.printf(
        debug_severity::DEBUG, "Quake: %d",
