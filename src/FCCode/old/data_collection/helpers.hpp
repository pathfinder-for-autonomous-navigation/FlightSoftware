 ...
 * @param buf_lock Readers-writers lock that needs to be obtained in order to
 * write to the buffer.
 * @param val_src_lock Readers-writers lock that needs to be obtained in order
 * to read the element's value.
 ...
 * @param val_src_lock Readers-writers lock that needs to be obtained in order
 * to read the element's value.
 * @param num_measurements Number of measurements that have been used in the
 * average.
 */
void add_to_avg(std::array<float, 3> &avg, const std::array<float, 3> &val,
 ...
 * @param avg Pointer to the current average. All of the elements of the average
 * will be set to zero.
 * @param num_measurements Pointer to the integer containing the number of
 * measurements in the average. This field will be set to zero.
 */
