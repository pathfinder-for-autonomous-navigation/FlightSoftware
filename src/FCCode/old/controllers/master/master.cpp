...
static void check_docking_switch() {
  ...
  State::write(State::Master::docking_switch_pressed,
               Devices::docking_switch->pressed(), master_state_lock);
...
static void master_loop() {
  ...
  case MasterState::DETUMBLE: {
    ...
    State::ADCS::ADCSState adcs_state =
        State::read(State::ADCS::adcs_state, State::ADCS::adcs_state_lock);
    ...
    if (State::read(State::Master::is_deployed, master_state_lock) &&
      ...
      State::write(State::ADCS::adcs_state,
                   State::ADCS::ADCSState::ADCS_DETUMBLE,
                   State::ADCS::adcs_state_lock);
    }

    if (State::ADCS::angular_rate() <
        Constants::ADCS::MAX_STABLE_ANGULAR_RATE) {
      State::write(State::ADCS::adcs_state, State::ADCS::ADCSState::POINTING,
                   State::ADCS::adcs_state_lock);
      State::write(State::ADCS::cmd_attitude, State::ADCS::cur_attitude,
                   State::ADCS::adcs_state_lock);
  ...
  case MasterState::INITIALIZATION_HOLD:
    // The apply_uplink() is continuously checking uplink for manual control
    // commands and mode shift command
    break;
  case MasterState::NORMAL: {
    ...
    case PANState::FOLLOWER: {
      ...
      chMtxUnlock(&eeprom_lock);
      State::write(State::Master::is_follower, true,
                   State::Master::master_state_lock);
      State::write(RTOSTasks::LoopTimes::GNC, (unsigned int)60000,
                   RTOSTasks::LoopTimes::gnc_looptime_lock);

      if (State::GNC::distance() <
          Constants::read(Constants::Piksi::CDGPS_RANGE)) {
        State::write(pan_state, PANState::FOLLOWER_CLOSE_APPROACH,
                     master_state_lock);
        break;
      }

      if (State::Quake::msec_since_last_sbdix() >=
          Constants::read(Constants::Quake::UPLINK_TIMEOUT)) {
        dbg.println(debug_severity::NOTICE,
                    "Detected STANDBY condition due to no uplink being "
                    "received in the last 24 hours.");
        State::write(pan_state, PANState::STANDBY, master_state_lock);
      }
    } break;
    case PANState::FOLLOWER_CLOSE_APPROACH: {
      State::write(RTOSTasks::LoopTimes::GNC, (unsigned int)10000,
                   RTOSTasks::LoopTimes::gnc_looptime_lock);
      if (Master::standby_needed())
        State::write(pan_state, PANState::STANDBY, master_state_lock);
    ...
    case PANState::STANDBY: {
      if (!State::read(State::Master::uplink_command_applied,
                       master_state_lock)) {
        Master::apply_uplink_commands();
        State::write(State::Master::uplink_command_applied, true,
                     master_state_lock);
      }
      ...
      bool adcs_is_pointing =
          State::read(State::ADCS::adcs_state, State::ADCS::adcs_state_lock);
      bool uplink_commanding_adcs =
          State::read(State::Quake::most_recent_uplink.command_adcs,
    ...
    case PANState::DOCKING: {
      ...
      chMtxUnlock(&eeprom_lock);
      State::write(State::ADCS::adcs_state, State::ADCS::ADCSState::ZERO_TORQUE,
                   State::ADCS::adcs_state_lock);
      State::write(State::Propulsion::propulsion_state,
      ...
      unsigned int docking_timeout =
          Constants::read(Constants::Master::DOCKING_TIMEOUT);
      chVTDoSetI(&Master::docking_timer, S2ST(docking_timeout),
                 Master::stop_docking_mode, NULL);
      bool docking_switch_pressed =
          State::read(State::Master::docking_switch_pressed, master_state_lock);
      if (docking_switch_pressed &&
          State::Hardware::check_is_functional(Devices::docking_switch)) {
        State::write(State::Master::pan_state, PANState::DOCKED,
                     master_state_lock);
      }
    } break;
    ...
    case PANState::PAIRED: {
      ...
      // TODO Modify uplink struct and applicator so that they can un-modify the
      // gains
      State::write(pan_state, PANState::STANDBY, master_state_lock);
    } break;
  ...
  case MasterState::SAFE_HOLD:
    // Don't do anything; this is already being handled by the safe hold
    // callback function!
    break;
  default: {
 ...
 *
 * Checks for satellite safehold conditions/previous boot conditions and sets
 * the initial state of the master controller appropriately.
 */
void master_init() {
  ...
  if (previous_boot_safehold) {
    dbg.println(debug_severity::NOTICE,
                "Previous boot ended in safehold mode! The system will start "
                "in safehold now.");
    Master::safe_hold();
    return;
  }
  ...
  EEPROM.get(EEPROM_ADDRESSES::INITIALIZATION_HOLD_FLAG,
             prevboot_initialization_hold);
  chMtxUnlock(&eeprom_lock);
  if (prevboot_initialization_hold) {
    dbg.println(debug_severity::NOTICE,
                "Previous boot ended in initialization hold mode! The system "
                "will start in initialization hold now.");
    Master::initialization_hold();
    return;
  }

  dbg.println(debug_severity::NOTICE,
              "Previous boot did not end in initialization or safe hold mode. "
              "Checking to see if a hold is necessary...");
  if (Master::safe_hold_needed()) {
    unsigned int boot_number =
        State::read(State::Master::boot_number, master_state_lock);
    if (boot_number == 1)
      Master::initialization_hold();
    else
      Master::safe_hold();
