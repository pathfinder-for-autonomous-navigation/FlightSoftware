...
void Master::safe_hold_timer(void *arg) {
  ...
  unsigned int safe_hold_timeout =
      State::read(Constants::Master::SAFE_HOLD_TIMEOUT,
                  Constants::changeable_constants_lock);
  while (time_elapsed < safe_hold_timeout) {
    // If no longer in safe hold, don't keep running down the timer!
    if (State::read(State::Master::master_state,
    ...
    dbg.printf(debug_severity::INFO,
               "Time remaining until safe hold wait completed: %d",
               safe_hold_timeout - time_elapsed);
    chThdSleepSeconds(1);
    time_elapsed++;
  }

  // Here we set autoexit to true, since we can only get to this point if we
  // have finished the safe hold timer.
  State::write(State::Master::autoexited_safe_hold, true,
               State::Master::master_state_lock);
