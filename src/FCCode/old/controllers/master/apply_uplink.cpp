...
static THD_FUNCTION(power_cycler, args) {
  ...
  if (pcargs->gomspace) {
    ...
    chMtxUnlock(
        &State::Hardware::gomspace_device_lock); // Note: this actually never
                                                 // gets called, since Gomspace
                                                 // reboots
    chThdExit((msg_t)0); // Note: this actually never gets called, since
                         // Gomspace reboots
  }

  if (pcargs->adcs_system) {
    Gomspace::cycler_arg_t cycler_args = {&State::Hardware::adcs_device_lock,
                                          Devices::adcs_system,
                                          Devices::Gomspace::DEVICE_PINS::ADCS};
    if (Gomspace::adcs_system_thread == NULL)
      Gomspace::adcs_system_thread = chThdCreateFromMemoryPool(
          &Gomspace::power_cycler_pool, "PCYCLER:ADCS",
          RTOSTasks::master_thread_priority, Gomspace::cycler_fn,
          (void *)&cycler_args);
  }
  if (pcargs->spike_and_hold) {
    Gomspace::cycler_arg_t cycler_args = {
        &State::Hardware::spike_and_hold_device_lock, Devices::spike_and_hold,
        Devices::Gomspace::DEVICE_PINS::SPIKE_AND_HOLD};
    if (Gomspace::spike_and_hold_thread == NULL)
      Gomspace::spike_and_hold_thread =
          chThdCreateFromMemoryPool(&Gomspace::power_cycler_pool, "PCYCLER:SPH",
                                    RTOSTasks::master_thread_priority,
                                    Gomspace::cycler_fn, (void *)&cycler_args);
  }
  if (pcargs->piksi) {
    Gomspace::cycler_arg_t cycler_args = {
        &State::Hardware::piksi_device_lock, Devices::piksi,
        Devices::Gomspace::DEVICE_PINS::PIKSI};
    if (Gomspace::piksi_thread == NULL)
      Gomspace::piksi_thread = chThdCreateFromMemoryPool(
          &Gomspace::power_cycler_pool, "PCYCLER:PIKSI",
          RTOSTasks::master_thread_priority, Gomspace::cycler_fn,
          (void *)&cycler_args);
  }
  if (pcargs->quake) {
    Gomspace::cycler_arg_t cycler_args = {
        &State::Hardware::quake_device_lock, Devices::quake,
        Devices::Gomspace::DEVICE_PINS::QUAKE};
    if (Gomspace::quake_thread == NULL)
      Gomspace::quake_thread = chThdCreateFromMemoryPool(
          &Gomspace::power_cycler_pool, "PCYCLER:QUAKE",
          RTOSTasks::master_thread_priority, Gomspace::cycler_fn,
          (void *)&cycler_args);
  }
...
void Master::apply_uplink_commands() {
  Comms::Uplink uplink =
      State::read(State::Quake::most_recent_uplink, State::Quake::uplink_lock);

  // Master state handling
  State::Master::MasterState ms =
      (State::Master::MasterState)uplink.master_state;
  State::Master::PANState ps = (State::Master::PANState)uplink.pan_state;
  bool was_safehold = State::read(State::Master::master_state,
  ...
  // Command state
  State::write(State::Master::master_state, ms,
               State::Master::master_state_lock);
  ...
  if (was_safehold && !is_safehold) {
    ...
    State::write(State::Master::autoexited_safe_hold, false,
                 State::Master::master_state_lock);
  }

  // Write "ignore error" bits
  rwMtxWLock(&FaultState::Propulsion::propulsion_faults_state_lock);
  FaultState::Propulsion::cannot_pressurize_outer_tank_ignored =
      uplink.cannot_pressurize_outer_tank_ignored;
  FaultState::Propulsion::destabilization_event.is_not_set =
      uplink.ignore_destabilized;
  FaultState::Propulsion::overpressure_event.is_not_set =
      uplink.ignore_overpressure;
  rwMtxWUnlock(&FaultState::Propulsion::propulsion_faults_state_lock);
  State::write(FaultState::Gomspace::vbatt_ignored, uplink.vbatt_ignored,
               FaultState::Gomspace::gomspace_faults_state_lock);
  rwMtxWLock(&FaultState::ADCS::adcs_faults_state_lock);
  FaultState::ADCS::all_magnetometers_faulty_ignore =
      uplink.all_magnetometers_faulty_ignore;
  ...
  if (is_standby) {
    ...
    State::ADCS::PointingFrame frame =
        (State::ADCS::PointingFrame)uplink.adcs_frame;
    State::write(State::ADCS::cmd_attitude, cmd_attitude,
                 State::ADCS::adcs_state_lock);
    State::write(State::ADCS::cmd_attitude_frame, frame,
                 State::ADCS::adcs_state_lock);
  }
  State::ADCS::ADCSGainState gain_state =
      (State::ADCS::ADCSGainState)uplink.adcs_gain_state;
  ...
  if (command_propulsion && is_standby) {
    ...
    State::write(State::Propulsion::firing_data.impulse_vector, impulse_vector,
                 State::Propulsion::propulsion_state_lock);
    State::write(State::Propulsion::firing_data.time, impulse_time,
                 State::Propulsion::propulsion_state_lock);
  }
  ...
  chThdCreateStatic(power_cycler_wA, sizeof(power_cycler_wA),
                    RTOSTasks::master_thread_priority, power_cycler,
                    (void *)&args);

  // Manage docking motor
  chThdCreateStatic(docking_motor_toggler_wA, sizeof(docking_motor_toggler_wA),
                    RTOSTasks::master_thread_priority, docking_motor_toggler,
                    (void *)&(uplink.docking_motor_mode));
}