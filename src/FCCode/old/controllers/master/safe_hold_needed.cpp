...
bool Master::safe_hold_needed() {
  bool autoexited_safe_hold =
      State::read(State::Master::autoexited_safe_hold, master_state_lock);
  if (autoexited_safe_hold)
    return false; // Don't set up safehold in this case.

  // Check battery voltage
  unsigned short int vbatt = State::read(State::Gomspace::gomspace_data.vbatt,
                                         State::Gomspace::gomspace_state_lock);
  bool vbatt_ignored =
      State::read(FaultState::Gomspace::vbatt_ignored,
  ...
  bool cannot_pressurize_tank =
      FaultState::Propulsion::cannot_pressurize_outer_tank;
  bool cannot_pressurize_tank_ignored =
  ...
  bool all_magnetometers_faulty_ignore =
      State::read(FaultState::ADCS::all_magnetometers_faulty_ignore,
                  FaultState::ADCS::adcs_faults_state_lock);
  rwMtxRLock(&State::ADCS::adcs_state_lock);
  all_magnetometers_faulty &=
      !State::ADCS::adcs_hat.at("magnetometer_1").is_functional;
  all_magnetometers_faulty &=
      !State::ADCS::adcs_hat.at("magnetometer_2").is_functional;
  rwMtxRUnlock(&State::ADCS::adcs_state_lock);
  if (all_magnetometers_faulty && !all_magnetometers_faulty_ignore)
    return true;
  ...
  if (motor_x_faulty && !motor_x_faulty_ignore)
    return true;
  if (motor_y_faulty && !motor_y_faulty_ignore)
    return true;
  if (motor_z_faulty && !motor_z_faulty_ignore)
    return true;

  // Check if all sun sensors are faulty
  bool all_ssa_faulty = true;
  bool all_ssa_faulty_ignore =
      State::read(FaultState::ADCS::all_ssa_faulty_ignore,
                  FaultState::ADCS::adcs_faults_state_lock);
  rwMtxRLock(&State::ADCS::adcs_state_lock);
  ...
  rwMtxRUnlock(&State::ADCS::adcs_state_lock);
  if (all_ssa_faulty && !all_ssa_faulty_ignore)
    return true;
