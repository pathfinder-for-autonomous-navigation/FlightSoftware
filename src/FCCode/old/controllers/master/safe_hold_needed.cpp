...
bool Master::safe_hold_needed() {
    ...
    unsigned short int vbatt =
        State::read(State::Gomspace::gomspace_data.vbatt, State::Gomspace::gomspace_state_lock);
    bool vbatt_ignored = State::read(FaultState::Gomspace::vbatt_ignored,
                                     FaultState::Gomspace::gomspace_faults_state_lock);
    if (vbatt < Constants::Gomspace::SAFE_VOLTAGE || !vbatt_ignored) return true;
    ...
    if (cannot_pressurize_tank || !cannot_pressurize_tank_ignored) return true;
