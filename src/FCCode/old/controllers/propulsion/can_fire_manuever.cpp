...
int PropulsionTasks::can_fire_manuever() {
  ...
  if (State::Hardware::check_is_functional(temp_sensor_inner))
    State::write(State::Propulsion::tank_inner_temperature,
                 temp_sensor_inner->get(), propulsion_state_lock);
  if (State::Hardware::check_is_functional(temp_sensor_outer))
    State::write(State::Propulsion::tank_outer_temperature,
                 temp_sensor_outer->get(), propulsion_state_lock);
  ...
  bool is_inner_tank_temperature_too_high =
      State::read(State::Propulsion::tank_inner_temperature,
                  propulsion_state_lock) >= 100 &&
      State::Hardware::check_is_functional(temp_sensor_inner);
  bool is_outer_tank_temperature_too_high =
      State::read(State::Propulsion::tank_outer_temperature,
                  propulsion_state_lock) >= 48 &&
      State::Hardware::check_is_functional(temp_sensor_outer);

  if (is_inner_tank_temperature_too_high ||
      is_outer_tank_temperature_too_high || is_outer_tank_pressure_too_high) {
    ...
    if (is_inner_tank_temperature_too_high) {
      State::write(
          FaultState::Propulsion::overpressure_event_id,
  ...
  if (current_time >
      firing_time - Constants::Propulsion::thruster_preparation_time()) {
  ...
  bool is_nighttime =
      !State::read(State::ADCS::is_sun_vector_determination_working,
                   State::ADCS::adcs_state_lock);
  bool has_firing_happened_in_nighttime =
      State::read(State::GNC::has_firing_happened_in_nighttime,
                  State::ADCS::adcs_state_lock);
