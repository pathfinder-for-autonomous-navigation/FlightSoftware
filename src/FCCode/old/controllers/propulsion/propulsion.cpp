...
static void propulsion_state_controller() {
  PropulsionState propulsion_state =
      State::read(State::Propulsion::propulsion_state, propulsion_state_lock);
  ...
  case PropulsionState::IDLE: {
    if (can_manuever == -1)
      State::write(State::Propulsion::propulsion_state,
                   PropulsionState::VENTING, propulsion_state_lock);
    else if (can_manuever == 1)
      State::write(State::Propulsion::propulsion_state,
                   PropulsionState::AWAITING_PRESSURIZATION,
                   propulsion_state_lock);
  } break;
  case PropulsionState::VENTING: {
    if (venting_thread == NULL || chThdTerminatedX(venting_thread))
      venting_thread = chThdCreateStatic(
          venting_thread_wa, sizeof(venting_thread_wa),
  ...
  case PropulsionState::AWAITING_PRESSURIZATION: {
    ...
    if (can_manuever == -1)
      ...
      State::write(State::Propulsion::propulsion_state, PropulsionState::IDLE,
                   propulsion_state_lock);
    else {
      unsigned int stop_pressurization_time_delta = Constants::read(
          Constants::Propulsion::STOP_PRESSURIZATION_TIME_DELTA);
      gps_time_t start_pressurization_time =
          State::Propulsion::firing_data.time - stop_pressurization_time_delta;
      if (State::GNC::get_current_time() > start_pressurization_time)
        State::write(State::Propulsion::propulsion_state,
                     PropulsionState::PRESSURIZING, propulsion_state_lock);
    }
  } break;
    ...
    if (can_manuever != 1) {
      ...
      if (can_manuever == -1)
        ...
        State::write(State::Propulsion::propulsion_state, PropulsionState::IDLE,
               ...
               chThdTerminatedX(pressurizing_thread))
      pressurizing_thread = chThdCreateStatic(
          pressurizing_thread_wa, sizeof(pressurizing_thread_wa),
  ...
  case PropulsionState::FIRING: {
    ...
    float tank_pressure =
        State::read(State::Propulsion::tank_pressure, propulsion_state_lock);
    ...
    if (firing_thread == NULL || chThdTerminatedX(firing_thread))
      firing_thread = chThdCreateStatic(
          firing_thread_wa, sizeof(firing_thread_wa),
          RTOSTasks::propulsion_thread_priority, firing_fn, NULL);
  } break;
  default: {
    // Uh oh, undefined mode; go to disabled mode since that's safest
    State::write(State::Propulsion::propulsion_state, PropulsionState::DISABLED,
                 propulsion_state_lock);
  } break;
  }
}
...
void RTOSTasks::propulsion_controller(void *arg) {
  ...
  dbg.println(debug_severity::INFO,
              "Propulsion controller process has started.");
  chThdCreateStatic(propulsion_loop_wa, sizeof(propulsion_loop_wa),
                    RTOSTasks::propulsion_thread_priority, propulsion_loop,
                    NULL);
  ...
  unsigned char preferred_valve =
      EEPROM.read(EEPROM_ADDRESSES::PREFERRED_INTERTANK_VALVE);
  chMtxUnlock(&eeprom_lock);
  State::write(State::Propulsion::intertank_firing_valve, preferred_valve,
               propulsion_state_lock);

  dbg.println(debug_severity::INFO, "Waiting for deployment timer to finish.");
  bool is_deployed =
      State::read(State::Master::is_deployed, State::Master::master_state_lock);
  if (!is_deployed)
