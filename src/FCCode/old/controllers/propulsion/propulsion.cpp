...
static void propulsion_state_controller() {
    ...
    switch (propulsion_state) {
        ...
        case PropulsionState::IDLE: {
            if (can_manuever == -1)
                State::write(State::Propulsion::propulsion_state, PropulsionState::VENTING,
                             propulsion_state_lock);
            else if (can_manuever == 1)
                State::write(State::Propulsion::propulsion_state,
                             PropulsionState::AWAITING_PRESSURIZATION, propulsion_state_lock);
        } break;
        case PropulsionState::VENTING: {
            if (venting_thread == NULL || chThdTerminatedX(venting_thread))
                venting_thread =
                    chThdCreateStatic(venting_thread_wa, sizeof(venting_thread_wa),
        ...
        case PropulsionState::AWAITING_PRESSURIZATION: {
            ...
            if (can_manuever == -1)
                ...
                unsigned int stop_pressurization_time_delta =
                    Constants::read(Constants::Propulsion::STOP_PRESSURIZATION_TIME_DELTA);
                ...
                if (State::GNC::get_current_time() > start_pressurization_time)
                    State::write(State::Propulsion::propulsion_state, PropulsionState::PRESSURIZING,
                                 propulsion_state_lock);
            }
        } break;
        case PropulsionState::PRESSURIZING: {
            if (can_manuever != 1) {
                ...
                pressurizing_thread =
                    chThdCreateStatic(pressurizing_thread_wa, sizeof(pressurizing_thread_wa),
        ...
        case PropulsionState::FIRING: {
            ...
            if (firing_thread == NULL || chThdTerminatedX(firing_thread))
                firing_thread =
                    chThdCreateStatic(firing_thread_wa, sizeof(firing_thread_wa),
...
void RTOSTasks::propulsion_controller(void *arg) {
    ...
    dbg.println(debug_severity::INFO, "Propulsion controller process has started.");
