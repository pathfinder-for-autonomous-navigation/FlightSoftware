...
static void intertank_fire() {
    ...
    unsigned char preferred_valve =
        State::read(State::Propulsion::intertank_firing_valve, propulsion_state_lock);
...
THD_FUNCTION(PropulsionTasks::pressurizing_fn, args) {
    ...
    for (int i = 0; i < 20; i++) {
        ...
        State::write(State::Propulsion::tank_pressure, Devices::pressure_sensor->get(),
    ...
    chThdSleepUntil(tf);
    State::write(State::Propulsion::propulsion_state, State::Propulsion::PropulsionState::FIRING,
