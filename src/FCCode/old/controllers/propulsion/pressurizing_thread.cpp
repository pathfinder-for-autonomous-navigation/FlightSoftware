...
static void intertank_fire() {
  ...
  unsigned char preferred_valve = State::read(
      State::Propulsion::intertank_firing_valve, propulsion_state_lock);
...
THD_FUNCTION(PropulsionTasks::pressurizing_fn, args) {
  ...
  unsigned int dt =
      (unsigned int)(firing_time - State::GNC::get_current_time());
  systime_t tf = t0 + MS2ST(dt);

  for (int i = 0; i < 20; i++) {
    intertank_fire();
    chThdSleepMilliseconds(WAIT_BETWEEN_PRESSURIZATIONS);
    State::write(State::Propulsion::tank_pressure,
                 Devices::pressure_sensor->get(), propulsion_state_lock);
    tank_pressure =
        State::read(State::Propulsion::tank_pressure, propulsion_state_lock);
  ...
  if (tank_pressure < PRE_FIRING_OUTER_TANK_PRESSURE) {
    ...
    State::write(FaultState::Propulsion::cannot_pressurize_outer_tank, true,
