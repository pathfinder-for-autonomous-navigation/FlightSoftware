...
static void send_data() {
  double impulse[3], gps_position[3], gps_velocity[3], gps_position_other[3],
      gps_velocity_other[3];

  float *impulse_f =
      State::read(firing_data.impulse_vector.data(), propulsion_state_lock);
  double *gps_pos =
      State::read(State::GNC::gps_position.data(), gnc_state_lock);
  double *gps_vel =
      State::read(State::GNC::gps_velocity.data(), gnc_state_lock);
  double *gps_pos_other =
      State::read(State::GNC::gps_position_other.data(), gnc_state_lock);
  double *gps_vel_other =
      State::read(State::GNC::gps_velocity_other.data(), gnc_state_lock);
  for (int i = 0; i < 3; i++) {
...
void RTOSTasks::system_output_controller(void *arg) {
  ...
  dbg.println(debug_severity::INFO,
              "System output controller process has started.");
