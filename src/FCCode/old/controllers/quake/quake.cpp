...
static void quake_loop() {
  // PCYCLER:QUAKE if failing. Do this for as many times as it takes for the
  ...
  if (!State::Hardware::check_is_functional(quake) &&
    ...
    Gomspace::cycler_arg_t cycler_args = {
        &State::Hardware::quake_device_lock, quake,
        Devices::Gomspace::DEVICE_PINS::QUAKE};
    // Start cycler thread
    Gomspace::quake_thread =
        chThdCreateFromMemoryPool(&Gomspace::power_cycler_pool, "PCYCLER:QUAKE",
  ...
  unsigned int hours_since_sbdix =
      State::Quake::msec_since_last_sbdix() / 1000 / 60 / 60;
  EEPROM.put(EEPROM_ADDRESSES::HOURS_SINCE_SBDIX, hours_since_sbdix);
  chMtxUnlock(&eeprom_lock);

  QuakeState quake_state =
      State::read(State::Quake::quake_state, quake_state_lock);
  switch (quake_state) {
  case QuakeState::WAITING: {
    unsigned int quake_wait_period =
        Constants::read(Constants::Quake::QUAKE_WAIT_PERIOD);
    chVTDoSetI(&waiting_timer, quake_wait_period, end_waiting, NULL);
  } break;
  case QuakeState::TRANSCEIVING: {
    if (Quake::transceiving_thread == NULL ||
        chThdTerminatedX(Quake::transceiving_thread)) {
      Quake::transceiving_thread = chThdCreateStatic(
          Quake::transceiving_thread_workingArea,
...
void RTOSTasks::quake_controller(void *arg) {
  ...
  dbg.println(debug_severity::INFO,
              "Quake radio controller process has started.");
