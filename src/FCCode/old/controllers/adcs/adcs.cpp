...
static void update_gain_constants() {
  ADCSControllers::AttitudePD::kd =
      (float)Constants::read(Constants::ADCS::ATTITUDE_CONTROLLER_KD);
  ADCSControllers::AttitudePD::kp =
      (float)Constants::read(Constants::ADCS::ATTITUDE_CONTROLLER_KP);
  ADCSControllers::MomentumControl::k =
      (float)Constants::read(Constants::ADCS::MOMENTUM_CONTROLLER_K);
...
static THD_FUNCTION(adcs_loop, arg) {
  ...
  while (true) {
    ...
    // If ADCS isn't working, power-cycle it. Do this for as many times as it
    // takes for the device
    ...
    if (!State::Hardware::check_is_functional(adcs_system) &&
      ...
      Gomspace::cycler_arg_t cycler_args = {
          &State::Hardware::adcs_device_lock, Devices::adcs_system,
    ...
    bool adcs_control_allowed =
        (State::ADCS::adcs_state != State::ADCS::ADCS_SAFE_HOLD);
    ...
    if (adcs_control_allowed) {
      ...
      case ADCSState::ADCS_DETUMBLE: {
        ...
        MomentumControl::magfield =
            ADCSControllers::Estimator::magfield_filter_body;
        MomentumControl::momentum =
            ADCSControllers::Estimator::htotal_filter_body;
        MomentumControl::update();
        State::write(State::ADCS::mtr_cmds, MomentumControl::moment,
                     adcs_state_lock);
        rwMtxRLock(&adcs_state_lock);
      ...
      case ADCSState::POINTING: {
        ...
        // Convert command attitude to delta quaternion and feed data to
        // controller
        quat_rot_diff(cmd_attitude_eci.data(), State::ADCS::cur_attitude.data(),
                      AttitudePD::deltaquat.data());
        AttitudePD::angrate = State::ADCS::cur_ang_rate;
        rwMtxRUnlock(&adcs_state_lock);

        AttitudePD::update();
        State::write(State::ADCS::rwa_torques, AttitudePD::torque,
                     adcs_state_lock);
        chMtxLock(&State::Hardware::adcs_device_lock);
        if (!State::Hardware::check_is_functional(Devices::adcs_system))
          adcs_system->set_rwa_mode(RWAMode::ACCEL_CTRL,
                                    AttitudePD::torque.data());
        chMtxUnlock(&State::Hardware::adcs_device_lock);
      } break;
...
void RTOSTasks::adcs_controller(void *arg) {
  ...
  chThdCreateStatic(update_adcs_hat_workingArea,
                    sizeof(update_adcs_hat_workingArea),
                    RTOSTasks::adcs_thread_priority, update_adcs_hat, NULL);
  ...
  if (!is_deployed)
    chThdEnqueueTimeoutS(&deployment_timer_waiting, S2ST(DEPLOYMENT_LENGTH));
