...
static unsigned int ssa_tries =
    0; // Number of consecutive loops that we've tried to
       // collect SSA data
void ADCSControllers::read_adcs_data() {
  ...
  for (int i = 0; i < 3; i++)
    Estimator::r_gps_eci[i] = State::GNC::gps_position[i];
  for (int i = 0; i < 4; i++)
    Estimator::q_gps_ecef[i] = State::GNC::ecef_to_eci[i];
  Estimator::time = (unsigned int)State::GNC::get_current_time();
  rwMtxRUnlock(&State::GNC::gnc_state_lock);
  rwMtxWUnlock(&adcs_state_lock);

  std::array<float, 3> rwa_speed_cmds_rd, rwa_ramps_rd, rwa_speeds_rd,
      gyro_data, mag_data;
  chMtxLock(&adcs_device_lock);
  rwMtxWLock(&adcs_state_lock);
  adcs_system->get_rwa(rwa_speed_cmds_rd.data(), rwa_speeds_rd.data(),
                       rwa_ramps_rd.data());
  ...
  if (ssa_mode == SSAMode::IN_PROGRESS) {
    ...
    chMtxUnlock(&adcs_device_lock);
    State::write(State::ADCS::ssa_vec, Estimator::sat2sun_sensor_body,
                 adcs_state_lock);
