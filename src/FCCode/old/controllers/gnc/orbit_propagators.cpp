...
static void propagate_self_orbit() {
  std::array<double, 3> gps_pos =
      State::read(State::GNC::gps_position, gnc_state_lock);
  std::array<double, 3> gps_vel =
      State::read(State::GNC::gps_velocity, gnc_state_lock);
  j8_propagate(gps_pos, gps_vel, 1.0 / GNC::ORBIT_PROPAGATOR_DELTA_T,
               scratchpad);
  ...
  if (in_nighttime && !now_in_nighttime) {
    ...
    State::write(State::GNC::has_firing_happened_in_nighttime, false,
                 gnc_state_lock);
...
static void propagate_other_orbit() {
  std::array<double, 3> gps_pos =
      State::read(State::GNC::gps_position_other, gnc_state_lock);
  std::array<double, 3> gps_vel =
      State::read(State::GNC::gps_velocity_other, gnc_state_lock);
  j8_propagate(gps_pos, gps_vel, 1.0 / GNC::ORBIT_PROPAGATOR_DELTA_T,
               scratchpad);
...
static void update_eci_to_lvlh() {
  // Compute ECI to LVLH
  std::array<double, 3> v_raw =
      State::read(State::GNC::gps_velocity, State::GNC::gnc_state_lock);
  std::array<double, 3> r_raw =
      State::read(State::GNC::gps_position, State::GNC::gnc_state_lock);
  pla::Vec3f v, r;
  for (int i = 0; i < 3; i++)
    v[i] = v_raw[i];
  for (int i = 0; i < 3; i++)
  ...
  for (int i = 0; i < 4; i++)
    eci_to_lvlh[i] = eci_to_lvlh_quat[i];
  State::write(State::GNC::eci_to_lvlh, eci_to_lvlh,
               State::GNC::gnc_state_lock);
}
