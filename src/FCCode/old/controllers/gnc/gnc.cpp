...
static void gnc_calculation() {
  ...
  if (vect_mag(firing_vector.data()) >=
      Constants::Propulsion::MAX_FIRING_IMPULSE)
  ...
  if (is_valid_firing && is_not_standby) {
    ...
    State::write(State::Propulsion::firing_data.time, firing_time,
                 propulsion_state_lock);
  } else {
...
void RTOSTasks::gnc_controller(void *arg) {
  ...
  GNC::orbit_propagator_thread =
      chThdCreateStatic(GNC::orbit_propagator_workingArea,
