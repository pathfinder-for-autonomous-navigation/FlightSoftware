 ...
 * @date 6 Feb 2018
 * @brief Contains the code that is the first to be run when starting up the
 * satellite. This code initializes all of the daughter processes that become
 * responsible for the handling of the satellite during its lifetime.
 */
...
static void start_satellite_processes() {
  ...
  // adcs_thread = chThdCreateStatic(adcs_controller_workingArea,
  // sizeof(adcs_controller_workingArea),
  //     adcs_thread_priority, adcs_controller, NULL);

  // dbg.println(debug_severity::INFO, "Starting Gomspace controller process.");
  // gomspace_thread = chThdCreateStatic(gomspace_controller_workingArea,
  // sizeof(gomspace_controller_workingArea),
  //     gomspace_thread_priority, gomspace_controller, NULL);

  dbg.println(debug_severity::INFO, "Starting Piksi controller process.");
  piksi_thread = chThdCreateStatic(
      piksi_controller_workingArea, sizeof(piksi_controller_workingArea),
      piksi_thread_priority, piksi_controller, NULL);

  dbg.println(debug_severity::INFO,
              "Starting GNC calculation controller process.");
  gnc_thread = chThdCreateStatic(gnc_controller_workingArea,
                                 sizeof(gnc_controller_workingArea),
                                 gnc_thread_priority, gnc_controller, NULL);

  dbg.println(debug_severity::INFO,
              "Starting system output controller process.");
  system_output_thread = chThdCreateStatic(
      system_output_controller_workingArea,
      sizeof(system_output_controller_workingArea),
...
#ifdef DEBUG_ENABLED
  (void)chThdCreateStatic(debug_workingArea, sizeof(debug_workingArea),
                          NORMALPRIO, debug_function, NULL);
#endif
}
...
void pan_system_setup() {
  ...
  // unsigned int boot_number = State::read(State::Master::boot_number,
  // State::Master::master_state_lock);
  // EEPROM.get(EEPROM_ADDRESSES::NUM_REBOOTS_H, boot_number);
  // State::write(State::Master::boot_number, boot_number++,
  // State::Master::master_state_lock);
  // EEPROM.put(EEPROM_ADDRESSES::NUM_REBOOTS_H, boot_number);
  // chMtxUnlock(&eeprom_lock);
  // dbg.printf(debug_severity::INFO, "This is boot #%d since the satellite left
  // the deployer.", State::Master::boot_number);
  ...
  //         sizeof(deployment_timer_workingArea),
  //         RTOSTasks::MAX_THREAD_PRIORITY + 1, deployment_timer_function,
  //         NULL);
