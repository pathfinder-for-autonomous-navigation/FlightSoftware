...
void exit_deployment_timer() {
  State::write(State::Master::is_deployed, true,
...
void deployment_timer_function(void *arg) {
  ...
  chMtxUnlock(&eeprom_lock);
  if (deployed) {
    dbg.println(debug_severity::NOTICE, "Deployment hold time constraint has "
  ...
  while (time_elapsed < DEPLOYMENT_LENGTH) {
    ...
    dbg.printf(debug_severity::NOTICE,
               "Time remaining until deployment wait completed: %d",
