...
namespace GNC {
std::array<double, 3> gps_position, gps_position_other, gps_velocity,
    gps_velocity_other;
...
double distance() {
  ...
  std::array<double, 3> pos_other =
      State::read(gps_position_other, gnc_state_lock);
  pla::Vec3d dpos;
  for (int i = 0; i < 3; i++)
    dpos[i] = pos[i] - pos_other[i];
  return dpos.length();
}
...
std::array<double, 3> recorded_gps_position, recorded_gps_position_other,
    recorded_gps_velocity;
gps_time_t recorded_gps_position_time, recorded_gps_position_other_time,
    recorded_gps_velocity_time;
