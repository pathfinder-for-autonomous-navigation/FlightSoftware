 ...
 * @date 6 Feb 2018
 * @brief Contains definitions for enums used within the flight controller
 * state.
 */
...
//! PAN-Specific state of finite-state machine that's part of the master
//! controller
enum PANState {
    // Common states.
...
struct DeviceState {
    ...
    bool is_functional;
    //! If true, the software ignores that this device is nonfunctional and tries
    //! to use it anyway
    bool error_ignored;
...
enum ADCSState { ZERO_TORQUE, ADCS_DETUMBLE, POINTING, ADCS_SAFE_HOLD };
//! Gains can either be set for an individual satellite or a dual-configuration
//! satellite
enum ADCSGainState { NORMAL, PAIRED };
//! Available frames for specifying a command atittude
enum PointingFrame { ECI, LVLH };
}  // namespace ADCS

namespace Propulsion {
enum PropulsionState { DISABLED, IDLE, VENTING, AWAITING_PRESSURIZATION, PRESSURIZING, FIRING };
...
namespace Quake {
enum QuakeState { WAITING, TRANSCEIVING };
}
}  // namespace State
