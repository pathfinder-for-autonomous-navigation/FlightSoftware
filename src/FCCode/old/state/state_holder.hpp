...
//! Are we able to collect the sun vector? i.e. the ADCS system is not timing
//! out on determining
// a sun vector, right?
extern bool is_sun_vector_collection_working;
/**
 * @brief Computes the magnitude of the angular rate of the spacecraft based on
 * the cur_ang_rate vector.
 * @return float The current angular rate of the spacecraft.
 */
inline float angular_rate() { return vect_mag(cur_ang_rate.data()); }
...
//! Most recently expected GPS position of other satellite, as last obtained
//! from Piksi or ground.
extern std::array<double, 3> recorded_gps_position_other;
//! GPS timestamp at which most recent position of other satellite was
//! collected.
...
//! Most recent GPS velocity of other satellite, as last obtained from the orbit
//! propagator.
...
//! Current propagated GPS time. Propagation occurs on each call of
//! current_time().
// This field needs to be updated every time GPS time is actually collected.
extern gps_time_t current_time;
//! Current propagated GPS time collection timestamp. Propagation occurs on each
//! call of current_time().
// This field needs to be updated every time GPS time is actually collected.
extern systime_t time_collection_timestamp;
//! Function to report (propagated) current time.
gps_time_t get_current_time();
//! Tracks whether or not a firing has happened during the current nighttime
//! period.
...
//! Last time at which an sbdix was successfully completed by the satellite.
//! Used in order
...
//! Packets are automatically added to this stack by the consumer threads if
//! they were
