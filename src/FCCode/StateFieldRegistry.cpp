...
void StateFieldRegistry::add_reader(const std::shared_ptr<Task>& reader, const std::shared_ptr<DataField>& field) {
...
void StateFieldRegistry::add_writer(const std::shared_ptr<Task>& writer, const std::shared_ptr<DataField>& field) {
    // Add field to registry if it doesn't exist
    if (_fields.find(field->name()) == _fields.end()) _fields.insert({field->name(), field});
...
bool StateFieldRegistry::can_read(const std::shared_ptr<Task>& reader, const std::shared_ptr<DataField>& field) {
    // TODO
    return false;
}

bool StateFieldRegistry::can_write(const std::shared_ptr<Task>& reader, const std::shared_ptr<DataField>& field) {
    // TODO
    return false;
}