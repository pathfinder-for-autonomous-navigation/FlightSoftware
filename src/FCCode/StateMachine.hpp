...
class StateMachine : public StateFieldRegistryReader<bool> {
  ...
  protected:
    ...
    std::map<std::pair<unsigned int, unsigned int>, std::shared_ptr<TransitionHandler>> _transition_handlers;
...
template<size_t num_states>
void StateMachine<num_states>::register_state_handler(unsigned int state, std::shared_ptr<StateHandler> handler) {
    if (!is_valid_state(state))
        printf(debug_severity::ERROR, "Invalid state number %d passed into function.", state);
