...
public:
  ...
  // protected:
  /**
   * @brief attempts to execute a step in the CONFIG command sequence. This
   * command sequence is executed on startup and executed whenever the
   * unexpected_flag is set
   */
  ...
  /**
   * @brief Pointer to the snapshot to be downlinked in pieces of 70 B, provided
   *by DownlinkProducer.
   **/
  ...
  QuakeControlTask &dbg_get_qct() { return qct; }
  size_t &dbg_get_max_snap() { return max_snapshot_size; }

  unsigned int &dbg_get_last_checkin() { return last_checkin_cycle; }

  char *dbg_get_mo_buffer_copy() { return mo_buffer_copy; }

  size_t &dbg_get_mo_idx() { return mo_idx; }

  bool &dbg_get_unexpected_flag() { return unexpected_flag; }
