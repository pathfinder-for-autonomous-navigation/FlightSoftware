...
with open('downlink_fields.csv') as csv_file:
    ...
    for row in csv_reader:
        if line_count != 0:
            field = {"group" : row[0], "name" : row[1], "type" : row[4], "size" : row[5], "min" : row[2], "max" : row[3]}
            FIELDS.append(field)
        line_count += 1
for field in FIELDS:
...
# std::trim_float(State::read(State::group::val, State::group::lock), MIN, MAX, representation_i);
...
def generate_field_code(field, i, packet_no):
    ...
    serializer_line = ""
    lock_name = field["group"] + "::" + field["group"][7:] + "_state_lock"
    if field["type"] == "bool":
        serializer_line = "{0}.set(0, State::read({1}, {2}));".format(bitset_name, field["name"], lock_name)
    elif field["type"] == "int":
        serializer_line = "trim_int(State::read({0},{1}), {2}, {3}, &{4});".format(field["name"], lock_name, field['min'], field['max'], bitset_name)
    elif field["type"] == "temperature":
        serializer_line = "trim_temperature(State::read({0},{1}), &{2});".format(field["name"], lock_name, bitset_name)
    elif field["type"] == "float":
        serializer_line = "trim_float(State::read({0},{1}), {2}, {3}, &{4});".format(field["name"], lock_name, field['min'], field['max'], bitset_name)
    elif field["type"] == "double":
        serializer_line = "trim_double(State::read({0},{1}), {2}, {3}, &{4});".format(field["name"], lock_name, field['min'], field['max'], bitset_name)
    elif field["type"] == "float vector" or field["type"] == "double vector":
        serializer_line = "trim_vector(State::read({0},{1}), {2}, {3}, &{4});".format(field["name"], lock_name, field['min'], field['max'], bitset_name)
    elif field["type"] == "quaternion":
        serializer_line = "trim_quaternion(State::read({0},{1}), &{2});".format(field["name"], lock_name, bitset_name)
    elif field["type"] == "gps time":
        serializer_line = "trim_gps_time(State::read({0},{1}), &{2});".format(field["name"], lock_name, bitset_name)
    else:
        err = "Field {0} does not have a valid type".format(field["name"])
        print(err)

    epilogue =   "for(int i = 0; i < {0}.size(); i++) {{".format(bitset_name) + "\n" \
         + "  packet_{0}.set(packet_{0}_ptr++,{1}[i]);".format(packet_no, bitset_name) + "\n" \
         + "}"
...
def header_generator(fields):
    ...
    FRAME_SIZE_BITS = sum([field["size"] for field in fields])
    ...
    header_str += "  constexpr unsigned int NUM_PACKETS = (unsigned int) ceil(FRAME_SIZE_BITS / PACKET_SIZE_BITS);\n"
...
#####################################
# Generates code for generating full series of packets.
#####################################
def packet_generator(fields):
    PACKET_SIZE_BITS = 70*8
    FRAME_SIZE_BITS = sum([field["size"] for field in fields])
    NUM_PACKETS = int(math.ceil((FRAME_SIZE_BITS + 0.0) / PACKET_SIZE_BITS))

    packet_generator_str = "#include \"downlink_packet_generator.hpp\"\n\n"
    packet_generator_str += "void Comms::generate_packets(std::bitset<PACKET_SIZE_BITS> (&packets)[NUM_PACKETS], unsigned int downlink_no) {\n"

    field_ptr = 0
    for packet_no in range(0, NUM_PACKETS):
        current_packet_size = 0

        packet_str  = "  std::bitset<PACKET_SIZE_BITS>& packet_{0} = packets[{0}];".format(packet_no) + "\n"
        packet_str  = "  unsigned int packet_{0}_ptr;".format(packet_no) + "\n"
        packet_str += "  std::bitset<32> downlink_num_repr(downlink_no);" + "\n"
        packet_str +=   "  for(int i = 0; i < 32; i++) {" + "\n" \
                    + "    packet_{0}.set(packet_{0}_ptr++, downlink_num_repr[i]);".format(packet_no) + "\n" \
                    + "  }" + "\n"
        current_packet_size += 32

        packet_str += "  std::bitset<8> packet_num_repr({0});".format(packet_no) + "\n"
        packet_str +=   "  for(int i = 0; i < 8; i++) {" + "\n" \
                    + "    packet_{0}.set(packet_{0}_ptr++, packet_num_repr[i]);".format(packet_no) + "\n" \
                    + "  }" + "\n"
        current_packet_size += 8

        for x in range(field_ptr, len(fields)):
            field = fields[x]
            if current_packet_size + field["size"] > PACKET_SIZE_BITS:
                break
...
implementation_content = packet_generator(FIELDS)
