...
protected:
  ...
  // shared pointer set by mission manager - tells control task to dock or
  // undock motor
  WritableStateField<bool> *docking_config_cmd_fp;

  // state field returns whether or not the spacecraft are docked with one
  // another
  Serializer<bool> docked_sr;
  ReadableStateField<bool> docked_f;

  // state field returns whether or not the motor is in the docking
  // configuration
  Serializer<bool> dock_config_sr;
  ReadableStateField<bool> dock_config_f;

  // state field returns whether or not the motor is turning/in the process of
  // (un)docking
