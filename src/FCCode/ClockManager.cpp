...
ClockManager::ClockManager(StateFieldRegistry &registry,
    ...
    : TimedControlTask<void>(registry, 0), control_cycle_start_time(),
      control_cycle_size(_control_cycle_size), control_cycle_count_sr(),
...
void ClockManager::execute() {
  if (has_executed) {
    sys_time_t earliest_start_time =
        control_cycle_start_time + control_cycle_size;
