...
template <typename T> class StateFieldFunctions {
public:
  /**
...
/**
 * @brief Dummy class so that we can create pointers of type DataField that
...
/**
 * @brief Registry of state fields and which threads have read/write access to
...
private:
  std::map<std::string, std::shared_ptr<DataField>> _fields;
  std::map<std::shared_ptr<Task>, std::vector<std::shared_ptr<DataField>>>
...
public:
   ...
   */
  void add_reader(const std::shared_ptr<Task> &r,
                  const std::shared_ptr<DataField> &field);

  /**
   * @brief Allows the specified Control Task to write to the specified state
   * field. If the field is not present in the registry yet, it is added.
   *
   * @param r Task
   * @param field Data field
   */
  void add_writer(const std::shared_ptr<Task> &w,
                  const std::shared_ptr<DataField> &field);
   ...
   */
  bool can_read(const std::shared_ptr<Task> &r,
                const std::shared_ptr<DataField> &field);
   ...
   */
  bool can_write(const std::shared_ptr<Task> &r,
                 const std::shared_ptr<DataField> &field);
};
 ...
 */
template <typename T> class StateFieldRegistryReader : public ControlTask<T> {
protected:
...
public:
   ...
   * @return false
   */
  bool can_read(const std::shared_ptr<DataField> &field) {

    return (this->_registry).can_read(std::shared_ptr<Task>(this), field);
  }
   ...
   * @return false
   */
  bool can_write(const std::shared_ptr<DataField> &field) {
    return (this->_registry).can_read(std::shared_ptr<Task>(this), field);
  }
};
 ...
 * or uplink packet.
 */
template <typename T> class StateField : public DataField, public Debuggable {
protected:
...
public:
  ...
  StateField(const std::string &name, StateFieldRegistry &reg)
      : DataField(name), Debuggable(), _ground_readable(false),
        _ground_writable(false), _val(), _registry(reg), _fetcher(nullptr),
        _checker(nullptr) {}
  ...
  virtual bool init(bool gr, bool gw,
                    typename StateFieldFunctions<T>::fetch_f fetcher =
                        StateFieldFunctions<T>::null_fetcher,
                    typename StateFieldFunctions<T>::sanity_check_f checker =
                        StateFieldFunctions<T>::null_sanity_check) {
    _ground_readable = gr;
    _ground_writable = gw;
    _fetcher = fetcher;
    _checker = checker;
  ...
  bool can_read(const std::shared_ptr<Task> &r) {
    if (!this->is_initialized())
      return false;
    return _registry.can_read(r, std::shared_ptr<DataField>(this));
  }
   ...
   * @return false If Control Task does not have write access to state field.
   */
  bool can_write(const std::shared_ptr<Task> &w) {
    if (!this->is_initialized())
      return false;
    return _registry.can_write(w, std::shared_ptr<DataField>(this));
  }
  ...
  bool add_writer(Task &w) {
    if (!this->is_initialized())
      return false;
    return _registry.add_writer(&w, this);
  }
  ...
  bool add_reader(Task &r) {
    if (!this->is_initialized())
      return false;
    return _registry.add_reader(&r, this);
  }
   ...
   */
  T get(const std::shared_ptr<Task> &getter) {
    if (!is_initialized())
      return _val;
    if (!can_read(getter))
      printf(debug_severity::ALERT,
             "Task %s illegally tried to read state field %s.",
   ...
   * @return True if succeeded, false if field is not initialized.
   */
  bool set(const std::shared_ptr<Task> &setter, const T &t) {
    if (!is_initialized())
      return false;
    if (!can_write(setter))
      printf(debug_severity::ALERT,
   ...
   * @return True if succeeded, false if field is not initialized.
   */
  bool fetch(const std::shared_ptr<Task> &setter) {
   ...
   */
  bool sanity_check(const std::shared_ptr<Task> &getter) const {
 ...
 */
template <typename T> class InternalStateField : public StateField<T> {
public:
   ...
   */
  bool
  init(typename StateField<T>::fetch_f fetcher = StateField<T>::null_fetcher,
       typename StateField<T>::sanity_check_f checker =
           StateField<T>::null_sanity_check) {
...
protected:
  std::shared_ptr<Serializer<T, U, compressed_sz>> _serializer;

public:
  SerializableStateField(const std::string &name, StateFieldRegistry &reg)
      : StateField<T>(name, reg), _serializer(nullptr) {}
  /**
   ...
   */
  bool init(bool gw, const std::shared_ptr<Serializer<T, U, compressed_sz>> &s,
            typename StateFieldFunctions<T>::fetch_f fetcher =
                StateFieldFunctions<T>::null_fetcher,
            typename StateFieldFunctions<T>::sanity_check_f checker =
                StateFieldFunctions<T>::null_sanity_check) {
...
public:
   ...
   * but not writable.)
   */
  bool init(const std::shared_ptr<Serializer<T, U, compressed_sz>> &s,
            typename StateFieldFunctions<T>::fetch_f fetcher =
                StateFieldFunctions<T>::null_fetcher,
            typename StateFieldFunctions<T>::sanity_check_f checker =
                StateFieldFunctions<T>::null_sanity_check) {
    return SerializableStateField<T, U, compressed_sz>::init(false, s, fetcher,
                                                             checker);
  }
};
...
public:
  ...
  bool init(const std::shared_ptr<Serializer<T, U, compressed_sz>> &s,
            ...
            typename StateFieldFunctions<T>::sanity_check_f checker =
                StateFieldFunctions<T>::null_sanity_check) {
    return SerializableStateField<T, U, compressed_sz>::init(true, s, fetcher,
                                                             checker);
  }
};
