...
MissionManager::MissionManager(StateFieldRegistry &registry,
                               unsigned int offset)
    : TimedControlTask<void>(registry, offset), mission_mode_sr(10),
      mission_mode_f("pan.mode", mission_mode_sr), is_deployed_sr(),
      is_deployed_f("pan.deployed", is_deployed_sr), sat_designation_sr(2),
  ...
  adcs_mode_fp =
      find_writable_field<unsigned char>("adcs.mode", __FILE__, __LINE__);
  adcs_cmd_attitude_fp =
      find_writable_field<f_quat_t>("adcs.cmd_attitude", __FILE__, __LINE__);
  adcs_ang_rate_fp =
      find_readable_field<float>("adcs.ang_rate", __FILE__, __LINE__);
  adcs_min_stable_ang_rate_fp = find_writable_field<float>(
      "adcs.min_stable_ang_rate", __FILE__, __LINE__);
