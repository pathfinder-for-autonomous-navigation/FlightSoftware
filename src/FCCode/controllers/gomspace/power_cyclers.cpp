...
THD_FUNCTION(Gomspace::cycler_fn, args) {
    ...
    dbg.println(debug_console::severity::INFO, "Incrementing cycle count.");
    ...
    dbg.println(debug_console::severity::INFO, "Checking if Gomspace is functional.");
    chMtxLock(cycler_args->device_lock);
        if (State::Hardware::check_is_functional(gomspace)) {
            dbg.println(debug_console::severity::INFO, "Gomspace is functional. Turning off device.");
            ...
            dbg.println(debug_console::severity::INFO, "Turning on device.");
            gomspace->set_single_output(cycler_args->pin, 1);
            chThdSleepMilliseconds(10);

            dbg.println(debug_console::severity::INFO, "Writing device status to state.");
            State::write(State::Hardware::hat.at(cycler_args->device).powered_on, 
                true, State::Hardware::hardware_state_lock);
            State::write(State::Hardware::hat.at(cycler_args->device).is_functional, 
                (cycler_args->device)->is_functional(), State::Hardware::hardware_state_lock);

            dbg.println(debug_console::severity::INFO, "Completed power cycle.");
        }
        else dbg.println(debug_console::severity::ERROR, "Gomspace is nonfunctional.");
