...
void RTOSTasks::stop_safehold() {
    State::write_state(State::Master::master_state, State::Master::MasterState::DETUMBLE, master_state_lock);
    State::write_state(State::Master::pan_state, State::Master::PANState::MASTER_DETUMBLE, master_state_lock);
    // Here we set autoexit to false, since this function could be called by an uplink packet
    State::write_state(State::Master::autoexited_safe_hold, false, State::Master::master_state_lock); 
    ...
    State::write_state(State::Propulsion::propulsion_state, State::Propulsion::PropulsionState::IDLE, State::Propulsion::propulsion_state_lock);
...
void Master::safe_hold(unsigned short int reason) {
    ...
    State::write_state(State::Master::master_state, State::Master::MasterState::SAFE_HOLD, master_state_lock);
    State::write_state(State::Master::pan_state, State::Master::PANState::MASTER_SAFEHOLD, master_state_lock);

    // Disable ADCS
    State::write_state(State::ADCS::adcs_state, State::ADCS::ZERO_TORQUE, State::ADCS::adcs_state_lock);
    // Disable propulsion firings
    State::write_state(State::Propulsion::propulsion_state, State::Propulsion::PropulsionState::DISABLED, State::Propulsion::propulsion_state_lock);
