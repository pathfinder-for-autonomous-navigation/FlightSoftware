...
static void piksi_read() {
    dbg.println("Reading Piksi data");
...
void RTOSTasks::piksi_controller(void *arg) {
    ...
    dbg.println("Piksi controller process has started.");
    ...
    while(true) {
        ...
        if (!State::Hardware::check_is_functional(piksi)) {
            dbg.println("Piksi not functional.");
            if (!is_fake_piksi) {
                dbg.println("Power cycling Piksi.");
