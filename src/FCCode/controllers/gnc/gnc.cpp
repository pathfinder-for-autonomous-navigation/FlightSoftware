...
static void gnc_calculation() {
    ...
    if (is_valid_firing && is_not_standby) {
        dbg.println(debug_console::severity::INFO, "Writing recommended firing to state.");
        ...
        dbg.println(debug_console::severity::NOTICE, "No firing recommended.");
    }
}
...
void RTOSTasks::gnc_controller(void* arg) {
    ...
    while (true) {
        ...
        dbg.println(debug_console::severity::INFO, "Running GNC calculation.");
        gnc_calculation();
