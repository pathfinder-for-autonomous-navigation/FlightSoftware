...
inline constexpr size_t compressed_state_size(size_t num_states) {
  return static_cast<size_t>(
      ceil(log(static_cast<float>(num_states)) / logf(2.0f)));
}
...
class SMStateSerializer : public Serializer<unsigned int, unsigned int,
                                            compressed_state_size(num_states)> {
public:
  using Serializer<unsigned int, unsigned int,
                   compressed_state_size(num_states)>::Serializer;
  bool init() {
    return Serializer<unsigned int, unsigned int,
                      compressed_state_size(num_states)>::init(0,
                                                               num_states - 1);
  }
};
...
template <size_t num_states>
class SMStateField
    : public WritableStateField<unsigned int, unsigned int,
                                compressed_state_size(num_states)> {
public:
  /**
  ...
  SMStateField(const std::string &name, StateFieldRegistry &reg)
      : WritableStateField<unsigned int, unsigned int,
                           compressed_state_size(num_states)>(name, reg),
        _state_names() {}
  ...
  void init(const std::array<std::string, num_states> &state_names,
    ...
    WritableStateField<unsigned int, unsigned int,
                       compressed_state_size(num_states)>::init(s, nullptr,
                                                                checker);
  }
