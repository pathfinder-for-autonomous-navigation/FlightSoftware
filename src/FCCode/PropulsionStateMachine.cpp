...
static temperature_t tank_inner_temp_fetcher() { return static_cast<temperature_t>(0); }

static temperature_t tank_outer_temp_fetcher() { return static_cast<temperature_t>(0); }

const std::array<std::string,
                 static_cast<unsigned int>(PropulsionStateMachine::state_t::num_states)>
    PropulsionStateMachine::state_names = {
        "safed",    "idle", "waiting_for_pressurize", "pressurizing", "waiting_for_inject",
        "injecting"};
...
bool PropulsionStateMachine::init(unsigned int initial_state) {
    ...
    auto temperature_slzr_ptr =
        std::shared_ptr<Serializer<temperature_t, temperature_t, SerializerConstants::temp_sz>>(
            &temperature_serializer);
    auto tank_pressure_slzr_ptr =
        std::shared_ptr<Serializer<float, float, tank_pressure_sz>>(&pressure_serializer);
    auto intertank_valve_slzr_ptr =
        std::shared_ptr<Serializer<bool, bool, SerializerConstants::bool_sz>>(
            &intertank_valve_serializer);
    auto firing_time_slzr_ptr =
        std::shared_ptr<Serializer<gps_time_t, bool, SerializerConstants::gps_time_sz>>(
            &firing_time_serializer);
    auto firing_vector_slzr_ptr =
        std::shared_ptr<Serializer<f_vector_t, float, prop_firing_sz>>(&firing_vector_serializer);

    // Initialize state variables
    AbortIfInitFail(tank_inner_temperature.init(temperature_slzr_ptr, tank_inner_temp_fetcher,
                                                tank_inner_temp_sanity_checker),
                    false);
    AbortIfInitFail(tank_outer_temperature.init(temperature_slzr_ptr, tank_outer_temp_fetcher,
                                                tank_outer_temp_sanity_checker),
                    false);
    AbortIfInitFail(
        tank_pressure.init(tank_pressure_slzr_ptr, pressure_fetcher, pressure_sanity_checker),
        false);
    AbortIfInitFail(
        intertank_valve.init(intertank_valve_slzr_ptr, StateFieldFunctions<bool>::null_fetcher,
                             StateFieldFunctions<bool>::null_sanity_check),
        false);
    AbortIfInitFail(
        firing_time.init(firing_time_slzr_ptr, StateFieldFunctions<gps_time_t>::null_fetcher,
                         firing_time_sanity_checker),
        false);
    AbortIfInitFail(
        firing_vector.init(firing_vector_slzr_ptr, StateFieldFunctions<f_vector_t>::null_fetcher,
                           firing_vector_sanity_checker),
        false);
    AbortIfInitFail(
        StateMachine<num_prop_states>::init(state_names, static_cast<unsigned int>(state_t::safed)),
        false);
...
#define RegisterTransitionHandler(state1, state2, transition)                                   \
    register_transition_handler(                                                                \
        static_cast<unsigned int>(state_t::state1), static_cast<unsigned int>(state_t::state2), \
        std::make_shared<PropulsionTransitionHandler<state_transition_t::transition>>(          \
...
#undef RegisterTransitionHandler
    ...
    for (std::map<std::pair<unsigned int, unsigned int>,
                  std::shared_ptr<TransitionHandler>>::iterator it = _transition_handlers.begin();
