...
Downlink::Downlink(const StateFieldRegistry& r, const std::vector<FlowData>& flow_data)
{
...
size_t Downlink::compute_downlink_size() const {
...
void Downlink::produce_downlink(const unsigned int cycle_count, char* dest) {
    ...
    dest[0] = bit_array::modify_bit(dest[0], 7, 1);
    downlink_frame_offset++;
    packet_offset++;

    // Add control cycle count
    for(size_t i = 1; i < 8; i++) {
        bit_array::modify_bit(dest[0], 7 - i, cycle_count >> (32 - i));
    }
    for(size_t i = 8; i < 32; i++) {
        bit_array::modify_bit(dest[i / 8], 7 - (i % 8), cycle_count >> (32 - i));
    }
    downlink_frame_offset += 32; // Cycle count size.
    packet_offset += 32;
    ...
    for(auto const& flow : flows) {
        ...
        add_field_bits_to_downlink_packet(flow_id_bits, dest, packet_offset,
                packet_counter, downlink_frame_offset);

        for(auto const& field : flow.field_list) {
            const bit_array& field_bits = field->get_bit_array();
            add_field_bits_to_downlink_packet(field_bits, dest, packet_offset,
                packet_counter, downlink_frame_offset);
        }
    }
}
