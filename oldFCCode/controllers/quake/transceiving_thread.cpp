...
void Quake::go_to_waiting() { State::write(quake_state, QuakeState::WAITING, quake_state_lock); }
...
static void get_latest_uplink(QuakeMessage *uplink) {
    while (is_mt_queued() > 0) {
        ...
        for (int i = 0; i < Constants::Quake::NUM_RETRIES; i++) {
            ...
            if (response != -1) break;
            unsigned int wait_between_retries =
                Constants::read(Constants::Quake::WAIT_BETWEEN_RETRIES);
            chThdSleepMilliseconds(wait_between_retries);
        }
        if (response == 0) {
            State::write(State::Quake::sbdix_time_received, State::GNC::get_current_time(),
