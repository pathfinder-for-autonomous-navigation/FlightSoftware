...
THD_FUNCTION(PropulsionTasks::firing_fn, args) {
    ...
    std::array<float, 3> impulse_vector_eci = State::read(
        State::Propulsion::firing_data.impulse_vector, State::Propulsion::propulsion_state_lock);
    ...
    for (int i = 2; i < 6; i++) {
        ...
        float magnitude_product =
            Constants::Propulsion::NOZZLE_VECTORS.at(i).length() * impulse_vector_body.length();
    ...
    for (int i = 2; i < 6; i++) {
        ...
        for (int j = 0; j < 3; j++)
            nozzle_vector_matrix[k][j] = Constants::Propulsion::NOZZLE_VECTORS.at(i)[j];
        k++;
    }
    ...
    if (State::Hardware::check_is_functional(spike_and_hold)) {
        ...
        dbg.println(debug_severity::NOTICE, "Could not complete firing because DCDC is off.");
    }
    chSysUnlock();
    ...
    State::write(State::Propulsion::propulsion_state, State::Propulsion::PropulsionState::IDLE,
