...
int PropulsionTasks::can_fire_manuever() {
    if (State::Hardware::check_is_functional(pressure_sensor))
        State::write(State::Propulsion::tank_pressure, pressure_sensor->get(),
                     propulsion_state_lock);
    if (State::Hardware::check_is_functional(temp_sensor_inner))
        State::write(State::Propulsion::tank_inner_temperature, temp_sensor_inner->get(),
                     propulsion_state_lock);
    if (State::Hardware::check_is_functional(temp_sensor_outer))
        State::write(State::Propulsion::tank_outer_temperature, temp_sensor_outer->get(),
                     propulsion_state_lock);

    bool is_outer_tank_pressure_too_high =
        State::read(State::Propulsion::tank_pressure, propulsion_state_lock) >= 48 &&
        State::Hardware::check_is_functional(pressure_sensor);
    bool is_inner_tank_temperature_too_high =
        State::read(State::Propulsion::tank_inner_temperature, propulsion_state_lock) >= 100 &&
        State::Hardware::check_is_functional(temp_sensor_inner);
    bool is_outer_tank_temperature_too_high =
        State::read(State::Propulsion::tank_outer_temperature, propulsion_state_lock) >= 48 &&
        State::Hardware::check_is_functional(temp_sensor_outer);

    if (is_inner_tank_temperature_too_high || is_outer_tank_temperature_too_high ||
        is_outer_tank_pressure_too_high) {
        State::write(FaultState::Propulsion::overpressure_event, State::GNC::get_current_time(),
    ...
    gps_time_t firing_time =
        State::read(State::Propulsion::firing_data.time, propulsion_state_lock);
    ...
    bool is_standby = State::read(State::Master::pan_state, State::Master::master_state_lock) ==
                      State::Master::PANState::STANDBY;
    bool is_nighttime = !State::read(State::ADCS::is_sun_vector_determination_working,
                                     State::ADCS::adcs_state_lock);
    bool has_firing_happened_in_nighttime =
        State::read(State::GNC::has_firing_happened_in_nighttime, State::ADCS::adcs_state_lock);
    if (is_nighttime && has_firing_happened_in_nighttime && !is_standby) {
        ...
        // We're now in daylight! So we can set the "has firing happened in
        // nighttime" flag to false since it'll be a while before we return to
        // nighttime.
