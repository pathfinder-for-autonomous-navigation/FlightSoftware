...
void Master::safe_hold_timer(void *arg) {
    ...
    unsigned int safe_hold_timeout =
        State::read(Constants::Master::SAFE_HOLD_TIMEOUT, Constants::changeable_constants_lock);
    while (time_elapsed < safe_hold_timeout) {
        ...
        dbg.printf(debug_severity::INFO, "Time remaining until safe hold wait completed: %d",
