...
static THD_FUNCTION(power_cycler, args) {
    ...
    if (pcargs->gomspace) {
        ...
        chMtxUnlock(&State::Hardware::gomspace_device_lock);  // Note: this actually never
    ...
    if (pcargs->adcs_system) {
        ...
        if (Gomspace::adcs_system_thread == NULL)
            Gomspace::adcs_system_thread = chThdCreateFromMemoryPool(
                &Gomspace::power_cycler_pool, "PCYCLER:ADCS", RTOSTasks::master_thread_priority,
                Gomspace::cycler_fn, (void *)&cycler_args);
    }
    if (pcargs->spike_and_hold) {
        Gomspace::cycler_arg_t cycler_args = {&State::Hardware::spike_and_hold_device_lock,
                                              Devices::spike_and_hold,
                                              Devices::Gomspace::DEVICE_PINS::SPIKE_AND_HOLD};
        if (Gomspace::spike_and_hold_thread == NULL)
            Gomspace::spike_and_hold_thread = chThdCreateFromMemoryPool(
                &Gomspace::power_cycler_pool, "PCYCLER:SPH", RTOSTasks::master_thread_priority,
                Gomspace::cycler_fn, (void *)&cycler_args);
    }
    if (pcargs->piksi) {
        Gomspace::cycler_arg_t cycler_args = {&State::Hardware::piksi_device_lock, Devices::piksi,
                                              Devices::Gomspace::DEVICE_PINS::PIKSI};
        if (Gomspace::piksi_thread == NULL)
            Gomspace::piksi_thread = chThdCreateFromMemoryPool(
                &Gomspace::power_cycler_pool, "PCYCLER:PIKSI", RTOSTasks::master_thread_priority,
                Gomspace::cycler_fn, (void *)&cycler_args);
    }
    if (pcargs->quake) {
        Gomspace::cycler_arg_t cycler_args = {&State::Hardware::quake_device_lock, Devices::quake,
                                              Devices::Gomspace::DEVICE_PINS::QUAKE};
        if (Gomspace::quake_thread == NULL)
            Gomspace::quake_thread = chThdCreateFromMemoryPool(
                &Gomspace::power_cycler_pool, "PCYCLER:QUAKE", RTOSTasks::master_thread_priority,
                Gomspace::cycler_fn, (void *)&cycler_args);
    }
...
void Master::apply_uplink_commands() {
    ...
    bool was_safehold =
        State::read(State::Master::master_state, State::Master::master_state_lock) ==
    ...
    chThdCreateStatic(power_cycler_wA, sizeof(power_cycler_wA), RTOSTasks::master_thread_priority,
                      power_cycler, (void *)&args);
