...
static void command_optimal_long_edge(const pla::Vec3f &r) {
    ...
    std::array<float, 3> sun_vector =
        State::read(ADCSControllers::Estimator::sat2sun_filter_body, adcs_state_lock);
    ...
    for (int i = 0; i < 4; i++)
        quat_from_triad(z.data(), long_edge_vectors[i].data(), r.get_data(), sun_vector.data(),
                        long_edge_quats[i].data());
...
void ADCSControllers::point_for_close_approach() {
    ...
    pla::Vec3f gps_position_relative =
        gps_position - gps_position_other;  // Negative of vector _to_ other satellite
    // Transform r from ECEF to ECI
