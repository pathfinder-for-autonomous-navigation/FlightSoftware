...
class PropStateMachineCase(SingleSatOnlyCase):
    ...
    def print_object(self):
        print(f"[TESTCASE] state: { Enums.prop_states.get_by_num(int(self.state))}")
    ...
    def fake_pressure(self, i):
        ...
        if self.state == str(Enums.prop_states.get_by_name("pressurizing")):
            if i > 100: 
                self.tank2_pressure = 26
    
    def fake_cycles_until_firing(self, i):
        if self.state == str(Enums.prop_states.get_by_name("await_firing")):
    ...
    def test_disabled_to_idle(self):
        ...
        self.state = Enums.prop_states.get_by_name("idle")
        self.cycle()
        assert self.state == str(Enums.prop_states.get_by_name("idle")), "[TESTCASE] failed: state != idle"
    ...
    def test_idle_to_await_pressurizing(self):
        ...
        self.cycle()
        assert self.state == str(Enums.prop_states.get_by_name("await_pressurizing")), "[TESTCASE] failed: state != await_pressurizing" 
    ...
    def test_await_pressurizing_to_pressurize(self):
        ...
        assert self.state == str(Enums.prop_states.get_by_name("pressurizing")), "[TESTCASE] failed: state != pressurizing"
        return
    ...
    def test_pressurize_to_await_firing(self):
        ...
        print(f"[TESTCASE] cycles_until_change: {self.cycle_until_change()}")
        assert self.state == str(Enums.prop_states.get_by_name("await_firing")), "[TESTCASE] failed: state != await_firing"
        return
    ...
    def test_await_firing_to_firing(self):
        ...
        print(f"[TESTCASE] cycles_until_change: {self.cycle_until_change()}")
        assert self.state == str(Enums.prop_states.get_by_name("firing")), "[TESTCASE] failed: state != firing"
        return
